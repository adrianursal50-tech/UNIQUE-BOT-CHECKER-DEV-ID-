...
# Update: Better field extraction (Location, Last Login, Collector
#         Points/Tier, Squad, Bindings, Hero History, Country,
#         Hero Activity) + Collector Tier mapping (Amateur → World).
# ===================================================================
...
# ────────────────────────────────────────────────────────────────
# SCREENSHOT-STYLE FORMATTING + JSON EXPORT
# ────────────────────────────────────────────────────────────────

# Skin quality -> tier bucket (heuristic mapping)
SKIN_TIER_MAP = {
    21: 'supreme',
    16: 'grand',
    17: 'grand',
...
def parse_skin_tiers(slist: list) -> Dict[str, int]:
...
def compute_account_age(created_ts) -> str:
    if not created_ts or not isinstance(created_ts, (int, float)) or created_ts <= 0:
        return "--"
    try:
...
# ─── Generic "look anywhere for a value" helper ─────────────────
def _is_blank(v) -> bool:
    return v is None or v == '' or v == 0 or v == [] or v == {} or v == "--" or v == "N/A"

def _scan_value(sources, tags=(), keys=(), default=None, max_depth=3):
    """
    Scan multiple dict sources for the first non-blank value.
    Searches by tag number, by string key, and nested (up to max_depth).
    """
    def _walk(src, depth):
        if depth > max_depth or not isinstance(src, dict):
            return None
        for t in tags:
            v = src.get(t)
            if not _is_blank(v):
...
def _to_str_list(val) -> str:
    """Convert a value (list/dict/str/int) to a comma-separated string."""
    if _is_blank(val): return "--"
    ...
    if isinstance(val, list):
        for item in val:
            if isinstance(item, str):
                ...
                for k in (1, 2, 0, 'name', 'hero_name'):
                    if k in item and isinstance(item[k], str):
                        out.append(item[k]); break
    return ", ".join(out) if out else "--"

# ─── Account payload builder ────────────────────────────────────
def build_account_payload(device_id: str, acc: int, zone: int, player_data: dict) -> Dict[str, Any]:
    v2l_raw = str(player_data.get('v2l_status', 'N/A')).lower()
    v2l_yes = v2l_raw in ('enabled', 'yes', '1', 'true', 'active')
    v2l_label = "Yes" if v2l_yes else "No"

    ban_raw = str(player_data.get('ban_status', 'NORMAL'))
    is_banned = 'ban' in ban_raw.lower()
    status_label = "Banned" if is_banned else "Valid"

    tiers = player_data.get('skin_tiers') or {
        'supreme': 0, 'grand': 0, 'exquisite': 0, 'deluxe': 0, 'exceptional': 0, 'common': 0
    }

    collector_pts = player_data.get('collector_points', 0) or 0
    try: collector_pts = int(collector_pts)
    except: collector_pts = 0
    collector_tier = player_data.get('collector_tier') or map_collector_tier(collector_pts)
    ...
    payload = {
        ...
        "player_info": {
            ...
            "current_heroes": player_data.get('hero_count', 0),
            "skins": player_data.get('skin_count', 0),
            "supreme_skins": tiers.get('supreme', 0),
            "grand_skins": tiers.get('grand', 0),
            "exquisite_skins": tiers.get('exquisite', 0),
            "deluxe_skins": tiers.get('deluxe', 0),
            "exceptional_skins": tiers.get('exceptional', 0),
            "common_skins": tiers.get('common', 0),
            "location": player_data.get('location', '--'),
            "last_login": player_data.get('last_login', '--'),
            "current_rank": player_data.get('current_rank', 'Unranked'),
            "high_rank": player_data.get('highest_rank', '--'),
            ...
            "bindings": player_data.get('bindings', '--'),
            "hero_history": player_data.get('hero_history', '--'),
        },
        "extended_info": {
            "country": player_data.get('country', '--'),
            "last_login_country": player_data.get('last_login_country', '--'),
            "creation_country": player_data.get('creation_country', '--'),
            "last_battle": player_data.get('last_battle', '--'),
            "account_age": player_data.get('account_age', '--'),
            "created_at": player_data.get('created_at', '--'),
            "featured_skin": player_data.get('featured_skin', '--'),
            "favourite_skins": player_data.get('favourite_skins', '--'),
            "recent_skin": player_data.get('recent_skin', 'Unknown'),
            "skin_log": player_data.get('skin_log', '0 skins tracked - most recent Unknown'),
        },
        "hero_activity": {
            "top_played": player_data.get('top_played', '--'),
...
def format_account_details_text(payload: Dict[str, Any]) -> str:
    ...
    lines.append(f"  Last Login: {p['last_login']} WIB")
    ...
    lines.append(f"  Last Battle: {ext['last_battle']} WIB")
    lines.append(f"  Account Age: {ext['account_age']}")
    lines.append(f"  Created At: {ext['created_at']} WIB")
...
def save_account(account_info: dict, player_data: dict, mode="detail", out_dir: str = None):
    ...
    try:
        payload = build_account_payload(device, acc, zone, player_data)
        json_str = format_account_details_json(payload)
        json_dir = os.path.join(base, FOLDERS["json"])
        os.makedirs(json_dir, exist_ok=True)
        json_path = os.path.join(json_dir, f"{acc}_{zone}_{device[:12]}.json")
...
def _extract_player_data(conn, account_id: int, zone_id: int, device_id: str) -> Optional[dict]:
    """
    Shared extraction logic for single + bulk.
    Attempts to pull as many fields as possible from:
      • skin_info   (pid 10144)
      • role_info   (pid 10129)
      • pd          (pid 11154 — player lookup)
      • login response extras (via conn)
    """
    ...
    # Core fields
    nick  = (_scan_value(sources, tags=(2,), keys=('nickname','name')) or f"Player_{account_id}")
    level = _scan_value(sources, tags=(3,), keys=('level',), default=1)

    skin_cnt = _scan_value(sources, tags=(10, 83), keys=('skin_count','skins'), default=0)
    hero_cnt = _scan_value(sources, tags=(9,),    keys=('hero_count','heroes','current_heroes'), default=0)

    cur_rank_val = _scan_value(sources, tags=(8, 6), keys=('current_rank','rank'), default=0) or 0
    max_rank_val = _scan_value(sources, tags=(95, 15), keys=('high_rank','highest_rank','max_rank'), default=0) or 0

    created_raw = _scan_value(sources, tags=(42,), keys=('created_at','creation_time','create_time')) or conn.creation_ts
    created_at  = "--"
    account_age = "--"
    ...
    # ─── Extended fields (best-effort; tags are heuristic) ───
    last_login_ts  = _scan_value(sources, tags=(46, 47, 55, 56, 60), keys=('last_login','last_login_time'))
    last_battle_ts = _scan_value(sources, tags=(48, 45, 57, 61),     keys=('last_battle','last_battle_time'))

    location_raw   = _scan_value(sources, tags=(44, 49, 51, 62, 63),
                                 keys=('location', 'address', 'region', 'city'))
    country_raw    = _scan_value(sources, tags=(43, 50, 52, 64, 65),
                                 keys=('country', 'nation'))
    last_login_country = _scan_value(sources, tags=(53, 66), keys=('last_login_country','login_country'))
    creation_country   = _scan_value(sources, tags=(54, 67), keys=('creation_country','create_country'))

    achievement_pts = _scan_value(sources, tags=(40, 41, 68), keys=('achievement_points','achievement'), default=0)
    collector_pts   = _scan_value(sources, tags=(34, 35, 69, 70, 71), keys=('collector_points','collector'), default=0)
    try: collector_pts = int(collector_pts or 0)
    except: collector_pts = 0
    collector_tier  = map_collector_tier(collector_pts)

    squad_raw       = _scan_value(sources, tags=(60, 72, 73, 74), keys=('squad','squad_name','team'))
    bindings_raw    = _scan_value(sources, tags=(55, 75, 76, 77), keys=('bindings','binding','linked'))

    # Hero activity
    hero_history_raw = _scan_value(sources, tags=(78, 79, 80, 81), keys=('hero_history','heroes_history','recent_heroes'))
    top_played_raw   = _scan_value(sources, tags=(82, 84, 85), keys=('top_played','top_heroes'))
    recent_match_raw = _scan_value(sources, tags=(86, 87, 88), keys=('recent_matches','matches'))

    # Skin info extras
    featured_skin   = _scan_value(sources, tags=(89, 90), keys=('featured_skin',))
    favourite_skins = _scan_value(sources, tags=(91, 93), keys=('favourite_skins','favorite_skins'))
    recent_skin     = _scan_value(sources, tags=(94, 96), keys=('recent_skin',))
    ...
    if not any(skin_tiers.values()):
        try: skin_tiers['common'] = int(skin_cnt or 0)
        except: skin_tiers['common'] = 0

    def _format_location(loc):
        if _is_blank(loc): return "--"
    ...
    player_data = {
        ...
        'last_online':      'N/A',
        'skin_breakdown':   parse_skin_breakdown(skin_list),
        ...
        'last_login':       _fmt_ts_wib(last_login_ts) if last_login_ts else "--",
        'achievement_points': int(achievement_pts or 0),
        'collector_points': collector_pts,
        'collector_tier':   collector_tier,
        'squad':            _to_str_list(squad_raw),
        'bindings':         _to_str_list(bindings_raw),
        'hero_history':     _to_str_list(hero_history_raw),
        'country':          _to_str_list(country_raw),
        'last_login_country': _to_str_list(last_login_country),
        'creation_country': _to_str_list(creation_country),
        'last_battle':      _fmt_ts_wib(last_battle_ts) if last_battle_ts else "--",
        'account_age':      account_age,
        'featured_skin':    _to_str_list(featured_skin),
        'favourite_skins':  _to_str_list(favourite_skins),
        'recent_skin':      _to_str_list(recent_skin) if not _is_blank(recent_skin) else "Unknown",
        'skin_log':         f"{int(skin_cnt or 0)} skins tracked - most recent Unknown",
        'top_played':       _to_str_list(top_played_raw),
        'recent_matches':   _to_str_list(recent_match_raw),
    }
    return player_data
...
def process_detail(device_id: str, account_id: int, zone_id: int, out_dir: str = None) -> Tuple[bool, Optional[dict]]:
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            player_data = _extract_player_data(conn, account_id, zone_id, device_id)
            if not player_data:
                return False, None
...
def run_bulk_job(job: dict, loop, app):
    ...
    def worker(device_id: str):
        ...
        if success and pd:
            ...
            try:
                ...
                os.makedirs(json_dir, exist_ok=True)
                json_path = os.path.join(json_dir, f"{acc}_{zone}_{device_id[:12]}.json")
                with open(json_path, "w", encoding="utf-8") as jf:
                    jf.write(format_account_details_json(payload))
            except Exception as e:
                log.debug(f"Bulk JSON save skipped: {e}")
    ...
    for attempt in range(4):
        try:
            ...
            edited = True
            log.info(f"Summary edit OK on attempt {attempt+1}")
            break
        except Exception as e:
    ...
    if not edited:
        for attempt in range(3):
            try:
                ...
                fut.result(timeout=30)
                log.info(f"Summary sent as new message on attempt {attempt+1}")
                break
            except Exception as e:
    ...
    if awaiting_single and auth:
        ...
        try:
            with GameConnection(device_id=device_id) as conn:
                ...
                player_data = _extract_player_data(conn, acc, zone, device_id)
                if not player_data:
                    await update.message.reply_text("❌ Failed to extract account data."); return
                ...
                payload   = build_account_payload(device_id, acc, zone, player_data)
                text_body = format_account_details_text(payload)
                json_body = format_account_details_json(payload)

                is_banned = 'ban' in str(player_data.get('ban_status', '')).lower()
                header    = "🎯 <b>HITS FOUND</b>"
                if is_banned:
                    header = "🎯 <b>HITS FOUND</b>  🔴 <b>BANNED</b>"
                ...
                try:
                    bio = io.BytesIO(json_body.encode('utf-8'))
                    bio.name = f"account_{acc}_{zone}.json"
                    await update.message.reply_document(
                        document=bio,
                        filename=f"account_{acc}_{zone}.json",
                        caption=f"📄 JSON export · `{acc}` ({zone})",
                        parse_mode="Markdown"
                    )
                except Exception as e:
                    log.error(f"JSON send failed: {e}")
                    await update.message.reply_text(
                        f"<b>📄 JSON export</b>\n<pre>{html_escape(json_body[:3500])}</pre>",
                        parse_mode="HTML"
                    )
