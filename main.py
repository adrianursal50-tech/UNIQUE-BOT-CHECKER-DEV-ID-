#!/usr/bin/env python3
# ===================================================================
# SHIN DevID Checker — Telegram Bot v2.0
# -------------------------------------------------------------------
#   • New fancy result card (JSON-backed)
#   • Live bulk stats (thread-safe)
#   • Watermark: @SHINRT771
...
def _heroes_list(hero_list) -> str:
    if not hero_list or not isinstance(hero_list, list): return "N/A"
    names = [str(h) for h in hero_list[:5]]
    return ", ".join(names) if names else "N/A"

def format_card_v2(device: str, data: dict, found_at: Optional[str] = None) -> str:
    """The fancy format the user asked for."""
    acc   = data.get("account_id", "?")
    zone  = data.get("zone_id", "?")
    nick  = data.get("nickname", "N/A")
    level = data.get("level", "N/A")
    heroes= data.get("hero_count", "N/A")
    skins = data.get("skin_count", "N/A")
    matches = data.get("matches", "N/A")

    bd = data.get("skin_breakdown", {}) or {}
    bd_block = _breakdown_to_block(bd)

    cur_rank = data.get("current_rank", "Unranked")
    max_rank = data.get("highest_rank", "N/A")

    collector     = data.get("collector", "N/A")
    coll_points   = data.get("collector_points", "N/A")
    achievement   = data.get("achievement", "N/A")
    location      = data.get("location", "NOT FOUND")
    last_login    = data.get("last_login", "N/A")
    login_ctry    = data.get("login_country", "N/A")
    reg_ctry      = data.get("reg_country", "N/A")
    squad         = data.get("squad", "N/A")
    hero_history  = data.get("hero_history", "N/A")
    if found_at is None:
...
def save_account_v2(account_info: dict, player_data: dict, out_dir: str = None) -> Optional[str]:
    """Save card + JSON. Returns path to JSON file if saved."""
...
def process_detail(device_id: str, account_id: int, zone_id: int, out_dir: str = None) -> Tuple[bool, Optional[dict]]:
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            matches = pd.get(4) or skin_info.get(4) or role_info.get(4) or "N/A"
            ...
            # These are best-effort guesses from common MLBB SDP tags.
            # Fall back to N/A if the field isn't present.
            coll_points   = pd.get(84) or skin_info.get(84) or "N/A"
            achievement   = pd.get(96) or skin_info.get(96) or "N/A"
            last_login    = pd.get(44) or skin_info.get(44)
            if isinstance(last_login, (int, float)) and last_login > 0:
                try:
                    last_login = datetime.fromtimestamp(last_login, tz=timezone.utc).astimezone(TZ_WIB).strftime("%Y-%m-%d %H:%M:%S")
                except: last_login = "N/A"
            else:
                last_login = "N/A"

            login_ctry = pd.get(28) or "N/A"
            reg_ctry   = pd.get(27) or "N/A"
            squad      = pd.get(62) or skin_info.get(62) or "N/A"
            ...
            player_data = {
                ...
                'login_country':    login_ctry,
                'reg_country':      reg_ctry,
                'squad':            squad,
                'hero_history':     [],
...
# ────────────────────────────────────────────────────────────────
# USER MANAGER (unchanged structure)
# ────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────
# LIVE STATS STATE (per job)
# ────────────────────────────────────────────────────────────────

class LiveStats:
    """Thread-safe live stats with rate/ETA tracking."""
    def __init__(self, total: int):
        ...
        self.recent = []   # rolling window for rate calc
...
def render_live(snap: dict, status: str = "RUNNING") -> str:
...
def run_bulk_job(job: dict, loop, app):
    ...
    def worker(device_id: str):
        ...
        if success and pd:
            ...
            # Full card in a code block so spacing holds
            pd['account_id'] = acc
            pd['zone_id']    = zone
            card = format_card_v2(device_id, pd)
            # Post as code block — plain text preserves spacing
            hit_msg = f"```\n{card}\n```"
...
# ────────────────────────────────────────────────────────────────
# BRUTE FORCE JOB (unchanged behaviour)
# ────────────────────────────────────────────────────────────────
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting_single and auth:
        ...
        try:
            with GameConnection(device_id=device_id) as conn:
                if not conn.login_to_login_server():
                    await msg.edit_text("❌ Failed to connect to login server."); return
                if not conn.get_game_server() or not conn.connect_to_game_server():
                    await msg.edit_text("❌ Failed to reach game server."); return
                ...
                last_login_raw = pd.get(44) or skin_info.get(44)
                last_login = "N/A"
                if isinstance(last_login_raw, (int, float)) and last_login_raw > 0:
                    try:
                        last_login = datetime.fromtimestamp(last_login_raw, tz=timezone.utc).astimezone(TZ_WIB).strftime("%Y-%m-%d %H:%M:%S")
                    except: pass

                v2l_text = ("Enabled" if str(v2l).lower() in ('enabled','yes','1','true')
                            else "Disabled" if str(v2l).lower() in ('disabled','no','0','false')
                            else "N/A")
                ...
                player_data = {
                    ...
                    'current_rank': map_rank(cur_rank_val),
                    'highest_rank': map_rank(max_rank_val) if max_rank_val else map_rank(cur_rank_val),
                    'ban_status': ban_stat, 'v2l_status': v2l_text,
                    'collector': "N/A", 'collector_points': coll_points,
                    'achievement': achievement, 'location': "NOT FOUND",
                    'last_login': last_login, 'login_country': login_ctry,
                    'reg_country': reg_ctry, 'squad': squad,
                    'hero_history': [],
                    'skin_breakdown': parse_skin_breakdown(skin_info.get(92, [])),
                    'account_id': acc, 'zone_id': zone,
                }
                save_account_v2(
                ...
                card = format_card_v2(device_id, player_data)
