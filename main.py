...
class GameLogin(BaseConnection):
    def __init__(self, device_id, stop_event: Optional[threading.Event] = None):
        ...
        self.platform = detect_platform(device_id)
        raw = device_id.strip()
        ...
        # For iOS, "android_id" slot is actually idfv, adid is idfa.
        # Use ios_usa channel + IDFA-first payload
        if self.platform == "ios":
            self.channel = CHANNEL_IOS
            self.auth_str = f'idfa={self.adid}&idfv={self.android}&device_unique_id={self.imei}'
        else:
            self.channel = CHANNEL_AND
            self.auth_str = f'gps_adid={self.adid}&android_id={self.android}&device_unique_id={self.imei}'
...
async def post_init(app):
    ...
    log.info(f"🚀 {BOT_NAME} v{BOT_VERSION} online — iOS + fast-stop + ban-sync")
...
def main():
    ...
    log.info("🔥 Starting SHIN Cyber Edition v5.0...")
    try:
