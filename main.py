...
Live stats + working BF kicker + ban reason translator.
"""
...
def kick_status_text(uid: int) -> str:
    ...
    if job.get("done"): status_label = "✅ FINISHED"
    elif s.get("status") == "fetching": status_label = "🔍 FETCHING PROFILE"
    elif s.get("status") == "error": status_label = "❌ ERROR"
    elif job.get("stop_event") and job["stop_event"].is_set() and not job.get("done"):
        status_label = "🛑 STOPPING"
    err = s.get("error")
    err_line = f"\n┣ ⚠️ Error  : <i>{html.escape(err[:60])}</i>" if err else ""
    return (
        f"⚡ <b>BRUTEFORCE KICKER</b> — {status_label}\n{LINE_HEAVY}\n"
        ...
        f"\n<i>Use /kickstatus to refresh · /kickstop to cancel</i>"
    )
...
def render_job(job: Job) -> str:
    ...
    eta_line = ""
    if job.speed > 0 and job.checked < total:
        remain = (total - job.checked) / job.speed
        eta_line = f"\n⏳ <b>ETA:</b> <b>{_fmt_secs(remain)}</b>  ·  ⚡ <b>{job.speed:.1f}/s</b>"
    latest = ""
    if job.latest:
        latest = "\n\n<b>Latest:</b>\n" + "\n".join(job.latest[-3:])
    ...
    return (
        ...
        f"⏱  Time:  <b>{_fmt_secs(job.elapsed)}</b>{eta_line}"
        f"{latest}\n\n"
            ...
            try:
                await bot.send_document(chat_id=chat_id,
                    ...
                    caption=(f"✅ <b>Done</b>\n"
                             f"Registered: <b>{job.registered}</b> │ "
                             f"Valid: <b>{job.valid}</b> │ "
                             f"Unregistered: <b>{job.unregistered}</b> │ "
                             f"Invalid: <b>{job.invalid}</b>\n"
                             f"⏱ {_fmt_secs(job.elapsed)}"), parse_mode=ParseMode.HTML)
            ...
            try:
                await bot.send_document(chat_id=chat_id,
                    ...
                    caption=(f"✅ <b>Ban scan done</b>\n"
                             ...
                             f"✅ Clean: <b>{job.valid - job.banned}</b> │ "
                             f"⚠️ Unregistered: <b>{job.unregistered}</b> │ "
                             f"❌ Invalid: <b>{job.invalid}</b>\n"
                             f"⏱ {_fmt_secs(job.elapsed)}"), parse_mode=ParseMode.HTML)
            except Exception as e: log.warning("send_document failed: %s", e)
...
async def cmd_kick(msg: Message, command: CommandObject):
    ...
    did = (command.args or "").strip()
    if not did:
        return await msg.answer("Usage: <code>/kick &lt;device_id&gt;</code>")
    with kick_lock:
    ...
    with kick_lock:
        kick_jobs[uid] = {
            "device_id": did, "rounds": KICK_ROUNDS,
    ...
    def _runner():
        try:
            ...
            log.exception("kick runner error: %s", e)
        finally:
            with kick_lock:
                j = kick_jobs.get(uid)
                if j: j["done"] = True
    ...
    async def _refresher():
        while True:
            ...
            with kick_lock:
                ...
                mid = j.get("message_id", ph.message_id)
            try:
                await bot.edit_message_text(
                    chat_id=ph.chat.id, message_id=mid,
...
async def cmd_kickstatus(msg: Message):
    if not storage.is_admin(msg.from_user.id):
        return await msg.answer("👑 Admin only.")
    await msg.answer(kick_status_text(msg.from_user.id),
                     reply_markup=kick_buttons(msg.from_user.id))
...
async def cmd_kickstop(msg: Message):
    ...
    job["stop_event"].set()
    await msg.answer("🛑 Stop signal sent — kick ends after current attempt.")
...
async def cb_ui_kickstatus(cq):
    ...
    await cq.message.answer(kick_status_text(cq.from_user.id),
                            reply_markup=kick_buttons(cq.from_user.id))
    await cq.answer()
