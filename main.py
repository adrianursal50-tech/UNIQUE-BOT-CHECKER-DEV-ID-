#!/usr/bin/env python3
# ===================================================================
# SHIN DevID Checker — Telegram Bot v3.0
# -------------------------------------------------------------------
#   • TCP lookup now parses ALL tags (collector/location/heroes/squad)
#   • Real MLBB hero ID → name mapping (130 heroes)
#   • Accurate rank thresholds from working tool
#   • Channel gate, admin live stats, structured zip
#   • Watermark: @SHINRT771
# ===================================================================
...
# ────────────────────────────────────────────────────────────────
# RANK MAPPING (exact thresholds from working tool)
# ────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────
# THE GOLD — extract every tag from PID 11154 response
# ────────────────────────────────────────────────────────────────

def extract_player_data(result):
    """Parse all tags from the PID 11153→11154 lookup response."""
    ...
    try:
        ...
        llc = pd.get(87, None)  # last login country
        cac = pd.get(97, None)  # create country
        ...
        # Affinity — tag 135 dict, key 1 = level
        aff = None
        t135 = pd.get(135, {})
        if isinstance(t135, dict):
            aff = t135.get(1)

        # Hero history — tag 91 is list of hero IDs
        heroes_list = []
        t91 = pd.get(91, [])
        if isinstance(t91, list):
            ...
            for hid in t91:
                ...
                seen.add(hid_i)
                heroes_list.append(hero_name(hid_i))
        ...
        return {
            ...
            "collector_point": cpt if cpt > 0 else None,
            "squad": squad,
            "affinity": aff,
            "total_battles": total,
            "win_rate": wr,
        ...
        log.error(f"extract_player_data error: {e}")
        return None
...
def _breakdown_to_block(bd):
    ...
    lines = []
    for k, l in [('supreme','Supreme'),('grand','Grand'),('exquisite','Exquisite'),
...
def format_card_v2(device, data, found_at=None):
    ...
    bindings = data.get("bindings") or "N/A"
    win_rate = data.get("win_rate")
    win_rate_str = win_rate if isinstance(win_rate, str) and win_rate != "N/A" else (
        f"{win_rate}%" if isinstance(win_rate, (int, float)) and win_rate > 0 else "N/A"
    )
    mvp = data.get("mvp"); mvp_str = f"{int(mvp):,}" if isinstance(mvp, (int,float)) and mvp > 0 else "N/A"
    hh = data.get("hero_history") or []
    hero_history = ", ".join(str(x) for x in hh[:5]) if hh else "N/A"
    ...
    return "\n".join([
        ...
        f"HERO HISTORY : {hero_history}", f"FOUND AT     : {found_at}",
        sep, f"👑 {BRAND}"
    ])
...
def card_to_json(device, data):
    return {
        ...
        "matches": data.get("matches"), "win_rate": data.get("win_rate"),
        "skin_breakdown": data.get("skin_breakdown", {}),
        "current_rank": data.get("current_rank"), "highest_rank": data.get("highest_rank"),
        "collector": data.get("collector_tier"), "collector_points": data.get("collector_point"),
        "location": data.get("location"), "last_login": data.get("last_login"),
        "login_country": data.get("login_country"), "reg_country": data.get("reg_country"),
        "squad": data.get("squad"), "hero_history": data.get("hero_history", []),
        "ban_status": data.get("ban_status"), "v2l_status": data.get("v2l_status"),
...
def process_detail(device_id, account_id, zone_id, out_dir=None, want_debug=False):
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            # Try to pull skin breakdown from lookup response
            try:
                plist = result.get(0)
                if isinstance(plist, list) and plist and isinstance(plist[0], dict):
                    player_data['skin_breakdown'] = parse_skin_breakdown(plist[0].get(92, []))
            except: pass
...
class UserManager:
    ...
    async def register_user(self, uid, username=None, first_name=None):
        ...
        if u not in self.users["users"]:
            self.users["users"][u] = {"username": username, "first_name": first_name,
    ...
    async def generate_key(self, duration, unit, qty=1, max_users=1):
        if unit in ('lifetime','l'):
            ...
            um = {'s':1,'m':60,'h':3600,'d':86400,'y':31536000,'hours':3600,'days':86400,'months':2592000,'lifetime':0}
        ...
        for _ in range(qty):
            ...
            self.keys["keys"][k] = {"created": datetime.now().isoformat(), "expiry": exp.isoformat(),
                "used_by": [], "duration": disp, "max_users": max_users, "dtype": unit, "dval": duration}
            keys.append(k)
    ...
    async def set_threads_limit(self, uid, limit):
    ...
    async def add_vip(self, uid):
        u = str(uid)
        if u in self.users["users"]:
            self.users["users"][u]["vip"] = True; self.users["users"][u]["activated"] = True
            await self._save_users(); return True
        return False
    async def remove_vip(self, uid):
        u = str(uid)
        if u in self.users["users"]:
            self.users["users"][u]["vip"] = False; await self._save_users(); return True
        return False
...
def render_welcome(first_name, is_admin, expiry_display, checked, hits):
    role = "👑  Admin — unlimited" if is_admin else f"🎟️  Access until: {expiry_display}"
    return (f"🎮  *DEV ID Checker Bot BY SHIN*\n{LINE_HEAVY}\nWelcome, *{first_name}*!\n\n{role}\n\n"
            ...
            f"📢  *Required Channels:*\n" + "\n".join(f"• [{c['name']}]({c['url']})" for c in REQUIRED_CHANNELS)
            + f"\n{LINE_HEAVY}\n👑 {BRAND}")
...
def _bar(pct, width=20):
    f = int(width * pct / 100); return "█" * f + "░" * (width - f)
...
def write_session_card(session_dir, device, acc, zone, pd, is_banned):
    try:
        ...
        if is_banned:
            ...
            fname = f"{_safe_name(acc)}_{_safe_name(zone)}_{_safe_name(nick)}.txt"
            body = format_card_v2(device, pd); extra = ""
...
async def cmd_start(update, ctx):
    ...
    if auth:
        ...
        kb = M([[B("🔥  Bulk Check", callback_data="tool_bulk")],
                [B("🔍  Single Account Check", callback_data="tool_single")],
                [B("📊  Statistics", callback_data="tool_stats")],
                [B("💳  Buy Key", callback_data="menu_buy"), B("📖  Help", callback_data="menu_help")]])
        if is_admin:
            kb = M(list(kb.inline_keyboard) + [[B("👑  ADMIN PANEL", callback_data="open_admin_panel")]])
        await update.message.reply_text(text, parse_mode="Markdown", reply_markup=kb)
    else:
        from telegram import InlineKeyboardButton as B, InlineKeyboardMarkup as M
        kb = M([[B("💳  Buy Access Key", callback_data="menu_buy")],
...
async def cmd_help(update, ctx):
    if not await gate(update, ctx): return
    await update.message.reply_text(
        f"📖  *DEV ID Checker — Help*\n{LINE_HEAVY}\n"
        f"🔥 Bulk Check — send `.txt` with device IDs\n"
        f"🔍 Single Check — send one device ID\n"
        f"📊 Statistics — hit counters\n\n"
        f"`/redeem <key>` · `/start` · `/stop`\n{LINE_HEAVY}\n👑 {BRAND}",
        parse_mode="Markdown")
...
async def cmd_status(update, ctx):
    ...
    with job_lock: job = active_jobs.get(uid)
    if job:
        await update.message.reply_text(f"⚡ Running — `{job.get('checked',0)}/{job.get('total',0)}`", parse_mode="Markdown")
    else: await update.message.reply_text("No active job.")
...
async def cmd_admin(update, ctx):
    from telegram import InlineKeyboardButton as B, InlineKeyboardMarkup as M
    kb = M([[B("🔑 Gen Key", callback_data="adm_genkey"), B("👥 Users", callback_data="adm_users")],
            [B("📊 Stats", callback_data="adm_stats"), B("⚡ Running", callback_data="adm_running")],
            [B("📢 Broadcast", callback_data="adm_broadcast_help")]])
    await update.message.reply_text("👑 *Admin Panel*", reply_markup=kb, parse_mode="Markdown")

@admin_only
async def cmd_raw(update, ctx):
    if not ctx.args:
        await update.message.reply_text("Usage: `/raw <device_id>`"); return
    did = " ".join(ctx.args).strip()
    msg = await update.message.reply_text(f"🔬 Dumping for `{did}`...", parse_mode="Markdown")
    ...
    def _dump():
        try:
            with GameConnection(device_id=did) as conn:
                conn.login_to_login_server(); conn.get_game_server(); conn.connect_to_game_server()
                conn.check_ban_status(); get_v2l_status(conn, acc, zone); conn.lookup_player(acc)
    ...
    await msg.edit_text(f"🔬 {len(chunks)} chunk(s)...", parse_mode="Markdown")
...
# ────────────────────────────────────────────────────────────────
# DOCUMENT / TEXT / CALLBACK HANDLERS
# ────────────────────────────────────────────────────────────────
...
async def on_text(update, ctx):
    ...
    msg = await update.message.reply_text(f"🔍 Checking `{did}`...", parse_mode="Markdown")
    acc, zone, st = await asyncio.to_thread(lambda: GameLogin(did).run())
    if not acc or not zone:
        await msg.edit_text(f"❌ Login failed: `{st}`", parse_mode="Markdown"); return
    ok, pd, _ = await asyncio.to_thread(process_detail, did, acc, zone, None, False)
    if not ok or not pd:
        await msg.edit_text(f"❌ Check failed for `{did}`", parse_mode="Markdown"); return
    pd['account_id'] = acc; pd['zone_id'] = zone
    card = format_card_v2(did, pd)
...
async def handle_callback(update, ctx):
    ...
    if data == "tool_single":
        ...
        await q.edit_message_text(f"🔍 *Single Check*\nSend the Device ID as text.", parse_mode="Markdown"); return
    if data == "tool_stats":
        with COUNTER_LOCK:
            lines = [f"📊 *Rank Hits*\n{LINE_HEAVY}"]
            for r in ['warrior','elite','master','gm','epic','legend','mythic']:
    ...
    if data == "open_admin_panel":
        ...
        kb = M([[B("🔑 Gen Key", callback_data="adm_genkey"), B("👥 Users", callback_data="adm_users")],
                [B("📊 Stats", callback_data="adm_stats"), B("⚡ Running", callback_data="adm_running")],
                [B("📢 Broadcast", callback_data="adm_broadcast_help")]])
        await q.edit_message_text("👑 *Admin Panel*", reply_markup=kb, parse_mode="Markdown"); return
    if data == "adm_running":
...
async def post_init(app):
    cmds = [
        ...
        BotCommand("stop", "Stop job"), BotCommand("status", "Status"),
        BotCommand("help", "Help"),
    ]
    await app.bot.set_my_commands(cmds)
    if OWNER_ID:
        await app.bot.set_my_commands(cmds + [
            BotCommand("admin", "Admin panel"), BotCommand("raw", "Dump raw responses"),
            BotCommand("genkey", "Generate key"), BotCommand("ban_user", "Ban user"),
            BotCommand("unban_user", "Unban user"), BotCommand("stats", "Statistics"),
