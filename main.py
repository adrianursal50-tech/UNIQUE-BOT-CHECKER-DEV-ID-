#!/usr/bin/env python3
# ===================================================================
# PREMIUM DEVID SEKER - TELEGRAM BOT v1.3
# Update: New detailed account format (screenshot layout) + JSON export
#         for both Single Check and Bulk Check.
# ===================================================================
...
# ────────────────────────────────────────────────────────────────
# NEW: SCREENSHOT-STYLE FORMATTING + JSON EXPORT
# ────────────────────────────────────────────────────────────────
...
def compute_account_age(created_ts) -> str:
    """Return account age like '5y 8m 27d - OG' from a unix timestamp."""
...
def build_account_payload(device_id: str, acc: int, zone: int, player_data: dict) -> Dict[str, Any]:
    """Central place to normalize all account data used by text+json formatters."""
    v2l_raw = str(player_data.get('v2l_status', 'N/A')).lower()
    v2l_yes = v2l_raw in ('enabled', 'yes', '1', 'true', 'active')
    v2l_label = "Yes" if v2l_yes else ("No" if v2l_raw in ('disabled','no','0','false','inactive') else "No")
...
def format_account_details_text(payload: Dict[str, Any]) -> str:
    """Returns the account info in the exact layout of the reference screenshot."""
...
def format_account_details_json(payload: Dict[str, Any]) -> str:
    """Returns the account info as pretty JSON matching the text layout."""
    obj = {
        "header": {
...
def telegram_pretty_block(text: str) -> str:
    """Wrap text in a Telegram <pre> block, HTML-escaped for safety."""
...
def format_account_card(device: str, acc: int, zone: int, player_data: dict) -> str:
    """Kept for file-save compatibility; new main display uses format_account_details_text."""
    lines = [
        "═" * 60,
        f"Device ID    : {device}",
        f"Account      : {acc} ({zone})",
...
def save_account(account_info: dict, player_data: dict, mode="detail", out_dir: str = None):
    ...
    # Always write JSON export (per-account) even for banned/other
    try:
...
def _extract_player_data(conn, account_id: int, zone_id: int, device_id: str) -> Optional[dict]:
    """Shared extraction logic used by both single check and bulk check."""
    ...
    # Optional extended fields (best effort from known tags)
    last_login_ts   = pd.get(46) or pd.get(47) or 0
    last_battle_ts  = pd.get(48) or pd.get(45) or 0
    location_raw    = pd.get(44) or pd.get(49) or ""
    country_raw     = pd.get(43) or pd.get(50) or ""

    skin_tiers = parse_skin_tiers(skin_info.get(92, []) if isinstance(skin_info, dict) else [])
    if not any(skin_tiers.values()):
        # Fallback: distribute skin_count into common if no per-skin data
        skin_tiers['common'] = int(skin_cnt or 0)
    ...
    player_data = {
        ...
        'last_online':      'N/A',
        'skin_breakdown':   parse_skin_breakdown(skin_info.get(92, []) if isinstance(skin_info, dict) else []),
        'skin_tiers':       skin_tiers,
        'device_id':        device_id,
        'account_id':       account_id,
        'zone_id':          zone_id,
        # Extended placeholders / best effort
    ...
    if awaiting_single and auth:
        ...
        try:
            with GameConnection(device_id=device_id) as conn:
                ...
                # Persist (this also writes JSON export under OUTPUT_DIR)
                save_account(
