...
class Storage:
    ...
    def _new_key():
        ...
        return "MLBB-" + "-".join("".join(secrets.choice(alpha) for _ in range(4)) for _ in range(4))
...
async def cmd_mykey(msg: Message):
    ...
    now = int(time.time())
    exp = (k["activated_at"] or 0) + k["duration_hours"] * 3600
    left = max(exp - now, 0)
...
async def cmd_check(msg: Message, command: CommandObject):
    ...
    if res.get("status") != "success":
        return await ph.edit_text(f"❌ <b>Failed</b>\n<code>…{html.escape(did[-12:])}</code>\n"
