#!/usr/bin/env python3
# ===================================================================
# DEV ID Checker — Telegram Bot v4.1  ◈  SHIN CYBER EDITION
# -------------------------------------------------------------------
...
def process_detail(device_id, account_id, zone_id, want_debug=False):
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            if not result:
                # Lookup empty — check ban status to see if banned
                try:
                    bs = conn.check_ban_status()
                    if 'ban' in str(bs).lower():
...
# ════════════════════════════════════════════════════════════════
# BULK JOB (30 threads, suppresses lookup_empty errors)
# ════════════════════════════════════════════════════════════════
...
def run_bulk_job(job, loop, app):
    ...
    def worker(did):
        ...
        if ok and pd:
            ...
            stats.inc("failed")
            # SUPPRESS lookup_empty error messages — only show real failures
            reason_str = reason or ""
            if reason_str and "lookup_empty" not in reason_str and "no_game_server" not in reason_str:
...
def run_validate_job(job, loop, app):
    ...
    def worker(did):
        ...
        if ok and pd:
            ...
            stats.inc("failed")
            r = reason or ""
            if "lookup_empty" in r:
                with valid_lock: invalid_list.append((did, "lookup_empty"))
            else:
                with valid_lock: invalid_list.append((did, r[:40] or "failed"))
        upd()
    ...
    total = snap["total"]
    valid_n = snap["hits"]
    banned_n = snap["banned"]
    invalid_n = snap["failed"]
...
def _start_validate_job(update, ctx, lines, uid):
    """Kick off real validator job."""
    threads = MAX_THREADS_DEFAULT
    with job_lock:
        if uid in active_jobs and active_jobs[uid].get("status") == "running":
            return False
    # This is called from async context, so spawn a thread via job
    job = {"user_id": uid, "chat_id": update.effective_chat.id,
...
async def _process_validate_input(update, ctx, text_or_data):
    """Real TCP validator — same as bulk check but with live stats."""
...
async def post_init(app):
    ...
    log.info(f"🚀 {BOT_NAME} v{BOT_VERSION} online — {MAX_THREADS_DEFAULT} threads default")
...
def main():
    ...
    log.info("🔥 Starting SHIN Cyber Edition v4.1...")
    try:
