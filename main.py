...
# Fixed: summary shown as new message, counter never gets stuck
# Added: lookup API fallback for location / last_login / collector_tier
# ===================================================================
...
# ── Lookup API (same endpoint used by checker.py) ──
...
# ────────────────────────────────────────────────────────────────
# FULL INFO LOOKUP  (fallback source for location / last_login / collector_tier)
# ────────────────────────────────────────────────────────────────
...
def _lookup_player(account_id, zone_id) -> Dict[str, Any]:
    """POST to the same lookup API checker.py uses."""
...
def _is_blank_str(v, blanks=("", "n/a", "na", "none", "unknown", "guest", "not found", "—")):
...
def fetch_full_info(account_id, zone_id) -> Dict[str, Any]:
    """
    Returns normalized dict from lookup API, or {"status": "error", ...}.
    Used as fallback to fill in fields the raw SDP protocol can't provide.
    """
    ...
    return {
        ...
        "location":         _safe_str(d.get("location"), "NOT FOUND"),
...
def apply_lookup_fallback(player_data: dict, full: Dict[str, Any]) -> dict:
    """
    Fill only the fields that the raw protocol couldn't provide.
    Never overwrites values that already look valid.
    """
    # location / last_login / collector_tier — always from lookup (protocol can't get them)
    if full.get("status") == "success":
        player_data['location']    = full.get("location", "NOT FOUND")
        player_data['last_online'] = full.get("last_login", "N/A")
        player_data['collector_tier']  = full.get("collector_tier", "N/A")
        player_data['collector_point'] = full.get("collector_point", 0)

        # Fallback for missing core fields
        if _is_blank_str(player_data.get('nickname')) or str(player_data.get('nickname','')).startswith('Player_'):
            if not _is_blank_str(full.get("nickname")):
                player_data['nickname'] = full["nickname"]

        try:
            lv = int(player_data.get('level') or 0)
        except (ValueError, TypeError):
            lv = 0
        if lv <= 1 and full.get("level"):
            player_data['level'] = full["level"]

        try:
            sk = int(player_data.get('skin_count') or 0)
        except (ValueError, TypeError):
            sk = 0
        if sk == 0 and full.get("skin_count"):
            player_data['skin_count'] = full["skin_count"]

        try:
            hc = int(player_data.get('hero_count') or 0)
        except (ValueError, TypeError):
            hc = 0
        if hc == 0 and full.get("hero_count"):
            player_data['hero_count'] = full["hero_count"]

        if player_data.get('current_rank', 'Unranked') == 'Unranked' and not _is_blank_str(full.get("current_rank")):
            player_data['current_rank'] = full["current_rank"]
        if _is_blank_str(player_data.get('highest_rank')) and not _is_blank_str(full.get("high_rank")):
            player_data['highest_rank'] = full["high_rank"]

        if not player_data.get('skin_breakdown'):
            player_data['skin_breakdown'] = full.get("skin_breakdown", {})
    else:
        # API failed — leave placeholders exactly like the old behavior
        player_data.setdefault('location', "NOT FOUND")
        player_data.setdefault('last_online', "N/A")
        player_data.setdefault('collector_tier', "N/A")
        player_data.setdefault('collector_point', 0)
...
# ════════════════════════════════════════════════════════════════
# ORIGINAL FORMAT (unchanged) + 3 new lines for the new fields
# ════════════════════════════════════════════════════════════════
def format_account_card(device: str, acc: int, zone: int, player_data: dict) -> str:
    lines = [
        ...
        f"Location     : {player_data.get('location', 'NOT FOUND')}",
        f"Last Login   : {player_data.get('last_online', 'N/A')}",
        f"Collector    : {player_data.get('collector_tier', 'N/A')} ({player_data.get('collector_point', 0)} pts)",
        "═" * 60,
    ]
    return "\n".join(lines)
...
def process_detail(device_id: str, account_id: int, zone_id: int, out_dir: str = None) -> Tuple[bool, Optional[dict]]:
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            # ── Fallback: fill N/A fields from public lookup API ──
...
def run_bulk_job(job: dict, loop, app):
    ...
    def worker(device_id: str):
        ...
        if success and pd:
            ...
            v2l_text = ("ACTIVE"   if str(pd.get('v2l_status','')).lower() in ('enabled','yes','1','true')
                        else "INACTIVE" if str(pd.get('v2l_status','')).lower() in ('disabled','no','0','false')
                        else "N/A")
            cur_rank     = pd.get('current_rank', 'Unranked')
            highest_rank = pd.get('highest_rank', 'N/A')
            heroes   = pd.get('hero_count', 0)
            skins    = pd.get('skin_count', 0)
            created  = pd.get('created_at', 'N/A')
            location   = pd.get('location', 'NOT FOUND')
            last_login = pd.get('last_online', 'N/A')
            coll_tier  = pd.get('collector_tier', 'N/A')
            coll_pt    = pd.get('collector_point', 0)

            if is_banned:
                ban_line = f"⚡ Account Status: 🔴 BANNED\n   {ban_stat}\n"
            else:
                ban_line = f"⚡ Account Status: 🟢 NORMAL\n"

            # ── Original format preserved + 3 new lines ──
            hit_msg = (
                f"🎯 *HITS FOUND*\n"
                f"📋 *ACCOUNT DETAILS (8\\-REQUEST FULL CHECK)*\n"
                f"{'═'*38}\n"
                f"{'═'*12}\n\n"
                f"📱 Device ID    : `{device_id}`\n"
                f"🆔 Account      : `{acc}` ({zone})\n"
                f"👤 Nickname     : *{nick}* (Lv\\.{level})\n"
                f"{ban_line}"
                f"🏆 Rank         : {cur_rank}\n"
                f"⭐ Max Rank     : {highest_rank}\n"
                f"🦸 Heroes       : {heroes}\n"
                f"🎨 Skins        : {skins}\n"
                f"🔐 V2L Status   : {v2l_text}\n"
                f"📍 Location     : {location}\n"
                f"🕒 Last Login   : {last_login}\n"
                f"💎 Collector    : {coll_tier} \\({coll_pt} pts\\)\n"
                f"📅 Created      : {created}\n"
                f"{'═'*38}\n"
                f"{'═'*12}\n\n"
        ...
        try:
            with GameConnection(device_id=device_id) as conn:
                ...
                player_data = {
                    ...
                    'created_at': created_at, 'last_online': 'N/A',
                    'skin_breakdown': parse_skin_breakdown(skin_info.get(92, [])),
                }
                ...
                # Re-derive display vars from possibly-updated player_data
                cur_rank = player_data.get('current_rank', 'Unranked')
                highest_rank = player_data.get('highest_rank', 'N/A')
                location   = player_data.get('location', 'NOT FOUND')
                last_login = player_data.get('last_online', 'N/A')
                coll_tier  = player_data.get('collector_tier', 'N/A')
                coll_pt    = player_data.get('collector_point', 0)
                ...
                details = (
                    ...
                    f"🦸 Heroes       : {hero_cnt}\n"
                    f"🎨 Skins        : {skin_cnt}\n"
                    f"🔐 V2L Status   : {'🟢 ACTIVE' if v2l_text=='ACTIVE' else '🔓 INACTIVE' if v2l_text=='INACTIVE' else 'N/A'}\n"
                    f"📍 Location     : {location}\n"
                    f"🕒 Last Login   : {last_login}\n"
                    f"💎 Collector    : {coll_tier} \\({coll_pt} pts\\)\n"
