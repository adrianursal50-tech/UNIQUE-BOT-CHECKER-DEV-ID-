#!/usr/bin/env python3
# ===================================================================
# PREMIUM DEVID SEKER - TELEGRAM BOT v3.0
# Full-info edition (merged from Dev.py)
# Features: Live stats, 10-thread bulk check, single check,
#           admin panel, key management, full account info
# ===================================================================
...
# ────────────────────────────────────────────────────────────────
# FULL PLAYER DATA PARSER (from Dev.py)
# ────────────────────────────────────────────────────────────────
...
def process_detail(device_id: str, account_id: int, zone_id: int, out_dir: str = None) -> Tuple[bool, Optional[dict]]:
    """Full check — returns rich player_data dict."""
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            # Merge ban status from login server if it was set there
...
def format_full_card(device_id: str, account_id: int, zone_id: int,
                     p: dict, gs_info: str = "N/A") -> str:
    """Full-info Markdown card for Telegram."""
    raw = device_id.strip()
    if raw.startswith(("and_", "ios_")): raw = raw[4:]
    ...
    level     = p.get('level', '?')
    country   = p.get('last_login_country', 'N/A')
    reg       = p.get('create_country', 'N/A')
    created   = p.get('last_login', 'N/A')
    location  = p.get('location', 'N/A')

    cur_rank  = p.get('current_rank', 'Unranked')
    high_rank = p.get('high_rank', 'Unranked')

    heroes    = p.get('hero_count', 0)
    skins     = p.get('skin_count', 0)
    collector = p.get('collector_tier', 'None')
    affinity  = p.get('affinity', 'None')

    battles   = p.get('total_battles', 0)
    wr        = p.get('win_rate', 'N/A')

    squad     = p.get('squad', '—')
    squad_id  = p.get('squad_id', 'N/A')

    ban_raw   = p.get('ban_status', 'Not Banned')
    ban_end   = p.get('ban_end', 'N/A')
    is_banned = any(x in str(ban_raw) for x in ('Ban', 'Suspend', 'Restrict'))
    ban_line  = (f"🔴 *{ban_raw}*"
                 + (f"\n   └ Ends: {ban_end}" if ban_end != 'N/A' else "")
                 if is_banned else "🟢 Not Banned")
    ...
    last_hero = lm.get('hero_name', 'N/A')
    prev_heroes = lm.get('prev', [])
    prev_line = ", ".join(prev_heroes[:5]) if prev_heroes else "—"
...
def save_account(account_info: dict, pd: dict, out_dir: str = None):
    ...
    card_text = format_full_card(device, acc, zone, pd, pd.get('gs_info', 'N/A'))

    rank_files = {
        "warrior": (FOLDERS["rank_warrior"], "warrior_hits.txt"),
        "elite":   (FOLDERS["rank_elite"],   "elite_hits.txt"),
        "master":  (FOLDERS["rank_master"],  "master_hits.txt"),
        "gm":      (FOLDERS["rank_gm"],      "grandmaster_hits.txt"),
        "epic":    (FOLDERS["rank_epic"],    "epic_hits.txt"),
        "legend":  (FOLDERS["rank_legend"],  "legend_hits.txt"),
        "mythic":  (FOLDERS["rank_mythic"],  "mythic_hits.txt"),
    }
...
# ────────────────────────────────────────────────────────────────
# BULK JOB (10 THREADS, LIVE STATS)
# ────────────────────────────────────────────────────────────────
...
def run_bulk_job(job: dict, loop, app):
    ...
    def worker(device_id: str):
        ...
        if success and pd:
            ...
            hit_msg = format_full_card(device_id, acc, zone, pd, pd.get('gs_info', 'N/A'))
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    card = format_full_card(device_id, acc, zone, pd, pd.get('gs_info', 'N/A'))
