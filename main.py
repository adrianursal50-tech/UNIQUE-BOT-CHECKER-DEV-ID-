#!/usr/bin/env python3
# ===================================================================
# PREMIUM DEVID SEKER - TELEGRAM BOT v3.4
# JSON output + step-by-step debug on process_detail
# ===================================================================
...
class GameConnection(BaseConnection):
    ...
    def get_game_server(self) -> bool:
        ...
        if pid == 6 and res:
            try:
                ...
                log.error(f"[GAMESRV] split failed: {e} tag1={res.get(1)}")
                return False
        log.error(f"[GAMESRV] unexpected pid={pid} res={res}")
        return False
    ...
    def connect_to_game_server(self) -> bool:
        ...
        for i in range(20):
            ...
            log.warning(f"[GSCONN] iter={i} pid={pid}")
    ...
    def lookup_player(self, search_value: int):
        ...
        for i in range(8):
            pid, res = self.recv_data()
            log.warning(f"[LOOKUP] iter={i} pid={pid} res_type={type(res).__name__}")
...
# ────────────────────────────────────────────────────────────────
# DETAIL CHECK  (with step-by-step debug)
# ────────────────────────────────────────────────────────────────
...
def process_detail(device_id: str, account_id: int, zone_id: int, out_dir: str = None) -> Tuple[bool, Optional[dict]]:
    ...
    try:
        ...
        conn.connect()
        log.warning(f"[DETAIL] step1 connect OK → {SERVER_HOST}:{SERVER_PORT}")

        if not conn.login_to_login_server():
            log.error("[DETAIL] step1 login_to_login_server FAILED")
            return False, None
        log.warning(f"[DETAIL] step1 login OK acc={conn.account_id} zone={conn.zone_id}")

        if not conn.get_game_server():
            log.error("[DETAIL] step2 get_game_server FAILED")
            return False, None
        log.warning(f"[DETAIL] step2 game_server = {conn.game_host}:{conn.game_port}")

        if not conn.connect_to_game_server():
            log.error("[DETAIL] step3 connect_to_game_server FAILED")
            return False, None
        log.warning("[DETAIL] step3 game handshake OK")

        skin_info = conn.get_skin_role_info(account_id, zone_id)
        log.warning(f"[DETAIL] step4 skin_info type={type(skin_info).__name__} keys={list(skin_info.keys()) if isinstance(skin_info, dict) else None}")

        ban_stat = conn.check_ban_status()
        log.warning(f"[DETAIL] step5 ban_stat={ban_stat}")

        v2l = get_v2l_status(conn, account_id, zone_id)
        log.warning(f"[DETAIL] step6 v2l={v2l}")

        result = conn.lookup_player(account_id)
        log.warning(f"[DETAIL] step7 lookup result type={type(result).__name__}")
        if isinstance(result, dict):
            log.warning(f"[DETAIL] step7 lookup keys={list(result.keys())}")
            if result.get(0):
                log.warning(f"[DETAIL] step7 result[0] type={type(result[0]).__name__} len={len(result[0])}")
                if isinstance(result[0], list) and len(result[0]) > 0:
                    log.warning(f"[DETAIL] step7 result[0][0] type={type(result[0][0]).__name__}")
                    if isinstance(result[0][0], dict):
                        log.warning(f"[DETAIL] step7 result[0][0] keys={list(result[0][0].keys())}")

        pd = extract_player_data(result) if result else None
        if not pd:
            log.error("[DETAIL] step8 extract_player_data returned None")
            return False, None
        log.warning(f"[DETAIL] step8 extracted: nick={pd.get('nickname')} lvl={pd.get('level')}")
        ...
        log.warning("[DETAIL] step9 saved OK")
        return True, pd
    except Exception as e:
        log.error(f"[DETAIL] EXCEPTION: {e}", exc_info=True)
        return False, None
    finally:
        if conn:
            try: conn.cleanup()
...
async def post_init(app: Application) -> None:
    ...
    log.info(f"🚀 {BOT_NAME} v{BOT_VERSION} online — full step logging ON")
