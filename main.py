...
class Storage:
    ...
    def activate_key(self, uid: int, key: str):
        k = self.get_key(key)
    ...
    def list_keys(self, limit: int = 20, offset: int = 0):
        return self._conn.execute(
            "SELECT * FROM keys ORDER BY created_at DESC LIMIT ? OFFSET ?", (limit, offset)
        ).fetchall()
    ...
    async def one(did: str):
        ...
        async with sem:
            ...
            if res.get("status") == "success":
                ...
                if p.get("is_banned"):
                    job.banned += 1
                    job.latest.append(fmt_result_line(False, did, p.get("ban_status", "Banned")))
                elif lvl < 9:
                    job.filtered += 1
        ...
        if job.results:
            ...
            try:
                await bot.send_document(
                    ...
                    caption=(f"✅ <b>Done</b>\n"
                             f"Valid: <b>{job.valid}</b> │ Banned: <b>{job.banned}</b> │ "
...
async def cmd_check(msg: Message, command: CommandObject):
    ...
    if p.get("is_banned"):
        if p.get("ban_reason"):
            ban_line += f"📝 Reason: <b>{html.escape(str(p['ban_reason']))}</b>\n"
        if p.get("ban_remaining"):
            ban_line += f"⏱ Remaining: <b>{html.escape(str(p['ban_remaining']))}</b>\n"
...
async def kg_count(msg: Message, state: FSMContext):
    try:
        n = int(msg.text.strip()); assert 1 <= n <= 500
    except Exception:
...
async def kg_duration(msg: Message, state: FSMContext):
    try:
        h = int(msg.text.strip()); assert 1 <= h <= 24 * 365
    except Exception:
...
async def kg_max(msg: Message, state: FSMContext):
    try:
        m = int(msg.text.strip()); assert 1 <= m <= 1_000_000
    except Exception:
