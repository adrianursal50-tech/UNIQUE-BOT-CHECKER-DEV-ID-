...
Commands:
  /check <id>    — single player info (NO ban)
  /bulk          — bulk player info (NO ban)
  /ban <id>      — single ban check
  /banbulk       — bulk ban check
...
async def cmd_start(msg: Message, command: CommandObject):
    ...
    txt = (f"👋 <b>Welcome, {html.escape(u.first_name or 'user')}!</b>\n\n"
           ...
           "• <code>/check &lt;id&gt;</code> — player info\n"
           "• <code>/bulk</code> — player info from .txt\n"
           "• <code>/ban &lt;id&gt;</code> — ban status only\n"
           "• <code>/banbulk</code> — ban scan from .txt\n"
...
async def cmd_help(msg: Message):
    await msg.answer(
        f"📖 <b>Help</b>\n\n"
        f"🔍 <code>/check and_xxxx…</code> — player info\n"
...
async def cmd_banbulk(msg: Message):
    ...
    await msg.answer(f"🚫 <b>Bulk ban scan</b>\nSend a <b>.txt</b> file "
                     f"(one device ID per line).\n⚡ {BULK_THREADS} threads.\n"
                     f"<i>Only ban status will be checked — much faster.</i>")
...
async def on_document(msg: Message):
    ...
    # Which bulk mode? default = player
    mode = "player"
    if msg.caption:
...
async def set_commands():
    cmds = [BotCommand(command="start",  description="Start"),
            ...
            BotCommand(command="check",  description="Single player check"),
