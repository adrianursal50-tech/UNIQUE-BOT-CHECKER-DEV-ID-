#!/usr/bin/env python3
# ===================================================================
# DEV ID Checker — Telegram Bot v3.5  ◈  SHIN CYBER EDITION
# -------------------------------------------------------------------
...
#   • FIX: full device ID display (no truncation)
#   • FIX: 3x retry + stage-specific failure reasons
#   • FIX: zone_id coercion, multi-packet PID 5 read, retries
#   • FIX: lookup FIRST (matches Dev.py order), then ban/v2l
#   • Watermark: @SHINRT771
# ===================================================================
...
class GameConnection(BaseConnection):
    ...
    def lookup_player(self, search_value):
        """Send PID 11153 and grab PID 11154. Retries once on timeout."""
        seen = []
...
# ════════════════════════════════════════════════════════════════
# DETAIL CHECK — lookup FIRST, ban/v2l after
# ════════════════════════════════════════════════════════════════
...
def process_detail(device_id, account_id, zone_id, want_debug=False):
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            # ── LOOKUP FIRST (matches Dev.py working order) ──
            ...
            # ── Ban + V2L afterwards (non-critical, don't kill the check) ──
            try:
            ...
            try:
                ...
                log.warning(f"v2l check failed: {e}")
                v2l = "N/A"
