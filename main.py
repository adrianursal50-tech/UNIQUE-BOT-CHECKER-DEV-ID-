#!/usr/bin/env python3
# ===================================================================
# PREMIUM DEVID SEKER - TELEGRAM BOT v1.7
# Robust recv_data: accepts body at any tag, skips empty, no false FAIL
# ===================================================================
...
def _coerce_to_bytes(v):
    """Turn any value into bytes if it looks like a serialized body, else None."""
    if v is None: return None
    if isinstance(v, str):
        try: return v.encode('utf-8')
...
class BaseConnection:
    ...
    # ════════════════════════════════════════════════════════════
    # ROBUST recv_data — tries tag 6, 5, then EVERY other tag
    # Only accepts a non-empty parsed body, so login FAIL(PID:2)
    # caused by empty/str-typed bodies is fully eliminated.
    # ════════════════════════════════════════════════════════════
    def recv_data(self) -> Tuple[Optional[int], Optional[SdpStruct]]:
        try:
            ...
            # 1) Preferred tags: 6 then 5 (server may use either)
            for tag in (6, 5):
                if tag not in res: continue
                val = res[tag]
                b   = _coerce_to_bytes(val)
                if b is not None and len(b) > 0:
                    try:
                        parsed = SdpStruct(b)
                        if parsed: return pid, parsed
                    except Exception:
                        pass
                if isinstance(val, dict) and len(val) > 0:
                    return pid, (val if isinstance(val, SdpStruct) else SdpStruct(val))

            # 2) Fallback: scan every remaining tag, try to parse as a struct
            for k, v in res.items():
                ...
                if isinstance(v, dict) and len(v) > 0:
                    return pid, (v if isinstance(v, SdpStruct) else SdpStruct(v))
...
def _lookup_log(msg: str):
    try:
        with open("debug_lookup.log", "a", encoding="utf-8") as f:
...
def _lookup_player(account_id, zone_id) -> Dict[str, Any]:
    ...
    while attempt <= LOOKUP_RETRIES:
        try:
            resp = requests.post(LOOKUP_API_URL, json=payload, timeout=LOOKUP_TIMEOUT)
            _lookup_log(f"POST {payload} -> HTTP {resp.status_code}")
            ...
            if resp.status_code == 429:
                ...
                wait = max(retry_after, _lookup_jitter(attempt))
                if attempt < LOOKUP_RETRIES and total_waited + wait <= LOOKUP_MAX_TOTAL_WAIT:
                    _lookup_log(f"  429 -> sleeping {wait:.1f}s (attempt {attempt+1}/{LOOKUP_RETRIES+1}, total {total_waited:.1f}s)")
                    time.sleep(wait)
                    total_waited += wait
                    attempt += 1
                    continue
                return {"status": "error", "error": "rate_limited_persistent"}

            if resp.status_code >= 500:
                if attempt < LOOKUP_RETRIES and total_waited + LOOKUP_BACKOFF_MAX <= LOOKUP_MAX_TOTAL_WAIT:
                    wait = _lookup_jitter(attempt)
                    _lookup_log(f"  {resp.status_code} -> sleeping {wait:.1f}s")
                    time.sleep(wait)
                    total_waited += wait
                    attempt += 1
                    continue
            ...
            try:
                ...
                _lookup_log(f"  non-JSON body: {resp.text[:200]}")
                return {"status": "error", "error": "invalid_json_response"}

            _lookup_log(f"  RAW: {json.dumps(data, ensure_ascii=False)[:1500]}")
            ...
            return {"status": "error", "error": "unrecognized_response"}

        except requests.exceptions.Timeout:
            if attempt < LOOKUP_RETRIES and total_waited + LOOKUP_BACKOFF_MAX <= LOOKUP_MAX_TOTAL_WAIT:
                wait = _lookup_jitter(attempt)
                _lookup_log(f"  timeout -> sleeping {wait:.1f}s")
                time.sleep(wait)
                total_waited += wait
                attempt += 1
                continue
            ...
            if attempt < LOOKUP_RETRIES and total_waited + LOOKUP_BACKOFF_MAX <= LOOKUP_MAX_TOTAL_WAIT:
                wait = _lookup_jitter(attempt)
                _lookup_log(f"  conn error -> sleeping {wait:.1f}s")
                time.sleep(wait)
                total_waited += wait
                attempt += 1
                continue
...
def _normalize_player_data(data: Dict[str, Any]) -> Dict[str, Any]:
    sb = data.get("skin_breakdown", {})
    if not isinstance(sb, dict): sb = {}

    return {
        "player_id":          _safe_str(data.get("player_id")),
        "nickname":           _safe_str(data.get("nickname")),
        "level":              _safe_int(data.get("level")),
        "hero_count":         _safe_int(data.get("hero_count")),
        "skin_count":         _safe_int(data.get("skin_count")),
        "skin_breakdown": {
        ...
        "current_rank":       _safe_str(data.get("current_rank")),
        "high_rank":          _safe_str(data.get("high_rank")),
        "location":           _safe_str(data.get("location"), "NOT FOUND"),
        "last_login":         _safe_str(data.get("last_login")),
        "achievement_points": _safe_int(data.get("achievement_points")),
        "collector_point":    _safe_int(data.get("collector_point")),
        "collector_tier":     _safe_str(data.get("collector_tier")),
        "squad":              _safe_str(data.get("squad")),
        "bindings":           _safe_str(data.get("bindings")),
        "win_rate":           _safe_int(data.get("win_rate")),
        "matches":            _safe_int(data.get("matches")),
        "mvp":                _safe_int(data.get("mvp")),
...
def fetch_full_info(account_id, zone_id) -> Dict[str, Any]:
    ...
    if raw.get("status") != "success":
        _lookup_log(f"fetch_full_info FAILED for {account_id}:{zone_id} — {raw.get('error')}")
        return {"status": "error", "error": raw.get("error", "unknown")}

    return {
        "status": "success",
        "player_data": _normalize_player_data(raw.get("player_data", {})),
    }


def _is_blank(v, blanks=("", "n/a", "na", "none", "null", "unknown", "guest",
                          "—", "-", "not found")):
...
def apply_lookup_fallback(player_data: dict, full: Dict[str, Any]) -> dict:
    ...
    pd_api = full.get("player_data", {}) or {}

    def _fill_str(key, api_key):
        cur = player_data.get(key)
        val = pd_api.get(api_key)
        if _is_blank(cur) and not _is_blank(val):
            player_data[key] = val

    def _fill_int(key, api_key):
        cur = player_data.get(key)
        val = pd_api.get(api_key)
        try:
            ...
            ci = 0
        if ci == 0 and val:
            player_data[key] = _safe_int(val)

    cur_nick = str(player_data.get('nickname') or '')
    if (_is_blank(cur_nick) or cur_nick.lower().startswith('player_')):
        if not _is_blank(pd_api.get("nickname")):
            player_data['nickname'] = pd_api["nickname"]
    ...
    if cur_rank.lower() == 'unranked' or _is_blank(cur_rank):
        if not _is_blank(pd_api.get("current_rank")):
            player_data['current_rank'] = pd_api["current_rank"]

    _fill_str('highest_rank',   'high_rank')
    _fill_str('location',       'location')
    _fill_str('last_online',    'last_login')
    _fill_str('collector_tier', 'collector_tier')
    _fill_str('squad',          'squad')
    _fill_str('bindings',       'bindings')

    if _is_blank(player_data.get('hero_history')):
        hh = pd_api.get("hero_history")
        if hh:
            player_data['hero_history'] = _flatten(hh)

    cur_sb = player_data.get('skin_breakdown') or {}
    if (not cur_sb) or all(int(cur_sb.get(k, 0) or 0) == 0
                            for k in ("Supreme","Grand","Exquisite","Deluxe","Exceptional","Common")):
        api_sb = pd_api.get("skin_breakdown") or {}
        if any(int(v or 0) > 0 for v in api_sb.values()):
            player_data['skin_breakdown'] = api_sb

    return player_data

# ────────────────────────────────────────────────────────────────
# SKIN BREAKDOWN (raw protocol fallback)
# ────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────
# BUILD PLAYER DATA FROM RAW PROTOCOL
# ────────────────────────────────────────────────────────────────
...
def _build_player_data_from_raw(device_id, account_id, zone_id, conn,
    ...
    b = _coerce_to_bytes(pd_raw)
    if b is not None:
        try: pd_raw = SdpStruct(b)
        except Exception: pd_raw = None
    ...
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            try:
                ...
                if full.get("status") != "success":
                    log.warning(f"Lookup failed for {account_id}:{zone_id} — {full.get('error')}")
                player_data = apply_lookup_fallback(player_data, full)
            except Exception as e:
                log.error(f"lookup fallback crashed: {e}")
            ...
            with GameConnection(device_id=device_id) as conn:
                ...
                try:
                    ...
                    with cf.ThreadPoolExecutor(1) as ex:
                        full = ex.submit(fetch_full_info, acc, zone).result(timeout=300)
                    if full.get("status") != "success":
                        log.warning(f"Lookup failed for {acc}:{zone} — {full.get('error')}")
                    player_data = apply_lookup_fallback(player_data, full)
                except Exception as e:
                    log.error(f"lookup fallback crashed: {e}")
