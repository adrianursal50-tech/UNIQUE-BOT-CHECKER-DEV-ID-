#!/usr/bin/env python3
# ===================================================================
# PREMIUM DEVID SEKER - TELEGRAM BOT v1.5
# Fix:  int() on SdpStruct crash (safe int casting everywhere)
# Add:  Ban Reason + Ban Remaining Time in text + JSON output
# ===================================================================
...
# ────────────────────────────────────────────────────────────────
# SAFE CASTING HELPERS  (fix for int() on SdpStruct)
# ────────────────────────────────────────────────────────────────

def _safe_int(val, default: int = 0) -> int:
    """Convert anything (int, float, str, bytes, dict, SdpStruct, list) to int safely."""
    if val is None:
        return default
    ...
    if isinstance(val, dict):
        # SdpStruct or dict — try common tag numbers / keys
        for k in (0, 1, 'value', 'val', 'count', 'points', 'level', 'amount'):
            if k in val:
                got = _safe_int(val[k], default=None) if default is not None else None
                if got is not None and got != 0:
                    return got
        # As a last resort, try first value
        for v in val.values():
            got = _safe_int(v, default=None)
            if got is not None and got != 0:
                return got
        return default
...
def _safe_str(val, default: str = "--") -> str:
    ...
    if isinstance(val, dict):
        for k in (1, 2, 0, 'name', 'value', 'text'):
            if k in val and isinstance(val[k], str):
                return val[k]
        return default
    if isinstance(val, (list, tuple)):
        if len(val) > 0:
            return _safe_str(val[0], default)
        return default
    return default
...
class GameConnection(BaseConnection):
    ...
    def check_ban_status(self) -> str:
        """Query ban info. Stores reason + remaining time on the connection."""
...
def _is_blank(v) -> bool:
    ...
    if isinstance(v, str) and v.strip() in ('', '--', 'N/A', 'None'): return True
...
def build_account_payload(device_id: str, acc: int, zone: int, player_data: dict) -> Dict[str, Any]:
    ...
    ban_reason    = str(player_data.get('ban_reason', '') or '')
    ban_remaining = str(player_data.get('ban_remaining', '') or '')

    tiers = player_data.get('skin_tiers') or {
        'supreme': 0, 'grand': 0, 'exquisite': 0, 'deluxe': 0, 'exceptional': 0, 'common': 0
    }

    collector_pts  = _safe_int(player_data.get('collector_points', 0), 0)
    achievement_pts= _safe_int(player_data.get('achievement_points', 0), 0)
    ...
    payload = {
        ...
        "v2l": v2l_label,
        "status": status_label,
        "ban_reason":    ban_reason if is_banned else "",
        "ban_remaining": ban_remaining if is_banned else "",
        "player_info": {
            "id": acc,
            "server": zone,
            "nickname": player_data.get('nickname', '--'),
            "level": _safe_int(player_data.get('level', 0), 0),
            "current_heroes": _safe_int(player_data.get('hero_count', 0), 0),
            "skins": _safe_int(player_data.get('skin_count', 0), 0),
            ...
            "achievement_points": achievement_pts,
...
def format_account_details_text(payload: Dict[str, Any]) -> str:
    ...
    # Ban info block (only when banned)
...
def format_account_card(device: str, acc: int, zone: int, player_data: dict) -> str:
    lines = [
        ...
        f"Status       : {player_data.get('ban_status', 'NORMAL')}",
...
def save_account(account_info: dict, player_data: dict, mode="detail", out_dir: str = None):
    ...
    skin   = _safe_int(player_data.get('skin_count', 0), 0)
...
def _extract_player_data(conn, account_id: int, zone_id: int, device_id: str) -> Optional[dict]:
    ...
    # Core
    nick  = _safe_str(_scan_value(sources, tags=(2,), keys=('nickname','name')), f"Player_{account_id}")
    level = _safe_int(_scan_value(sources, tags=(3,), keys=('level',), default=1), 1)

    skin_cnt = _safe_int(_scan_value(sources, tags=(10, 83), keys=('skin_count','skins'), default=0), 0)
    hero_cnt = _safe_int(_scan_value(sources, tags=(9,),    keys=('hero_count','heroes','current_heroes'), default=0), 0)

    cur_rank_val = _safe_int(_scan_value(sources, tags=(8, 6),  keys=('current_rank','rank'), default=0), 0)
    max_rank_val = _safe_int(_scan_value(sources, tags=(95, 15), keys=('high_rank','highest_rank','max_rank'), default=0), 0)

    created_raw = _safe_int(_scan_value(sources, tags=(42,), keys=('created_at','creation_time','create_time')) or conn.creation_ts, 0)
    created_at  = "--"
    account_age = "--"
    if created_raw > 0:
        try:
    ...
    location_raw       = _scan_value(sources, tags=(44, 49, 51, 62, 63), keys=('location', 'address', 'region', 'city'))
    country_raw        = _scan_value(sources, tags=(43, 50, 52, 64, 65), keys=('country', 'nation'))
    ...
    achievement_pts = _safe_int(_scan_value(sources, tags=(40, 41, 68), keys=('achievement_points','achievement'), default=0), 0)
    collector_pts   = _safe_int(_scan_value(sources, tags=(34, 35, 69, 70, 71), keys=('collector_points','collector'), default=0), 0)
    ...
    player_data = {
        ...
        'skin_log':         f"{skin_cnt} skins tracked - most recent Unknown",
