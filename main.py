...
# Output format = checker.py 8-REQ card
# N/A fields filled via public lookup API as fallback
# ===================================================================
...
def _is_blank(v, blanks=("", "n/a", "na", "none", "unknown", "guest", "—", "-")):
...
def fetch_full_info(account_id, zone_id) -> Dict[str, Any]:
    ...
    return {
        ...
        "login_country":      _safe_str(d.get("login_country")),
        "reg_country":        _safe_str(d.get("reg_country")),
        "hero_history":       _safe_str(d.get("hero_history")),
    }


def apply_lookup_fallback(player_data: dict, full: Dict[str, Any]) -> dict:
    """
    Fill any N/A / empty / zero fields in player_data from the lookup API.
    Never overwrites values that are already valid.
    """
    if full.get("status") != "success":
        return player_data

    # nickname
    nick = str(player_data.get('nickname') or '')
    if (_is_blank(nick) or nick.lower().startswith('player_')) and not _is_blank(full.get("nickname")):
        player_data['nickname'] = full["nickname"]

    # player_id
    if _is_blank(player_data.get('player_id')):
        player_data['player_id'] = full.get("player_id") or player_data.get('account_id')

    # level
    try: lv = int(player_data.get('level') or 0)
    except (ValueError, TypeError): lv = 0
    if lv <= 1 and full.get("level"):
        player_data['level'] = full["level"]

    # skins
    try: sk = int(player_data.get('skin_count') or 0)
    except (ValueError, TypeError): sk = 0
    if sk == 0 and full.get("skin_count"):
        player_data['skin_count'] = full["skin_count"]

    # heroes
    try: hc = int(player_data.get('hero_count') or 0)
    except (ValueError, TypeError): hc = 0
    if hc == 0 and full.get("hero_count"):
        player_data['hero_count'] = full["hero_count"]

    # matches
    try: mt = int(player_data.get('matches') or 0)
    except (ValueError, TypeError): mt = 0
    if mt == 0 and full.get("matches"):
        player_data['matches'] = full["matches"]

    # ranks
    cur = player_data.get('current_rank') or ''
    if (cur.lower() == 'unranked' or _is_blank(cur)) and not _is_blank(full.get("current_rank")):
        player_data['current_rank'] = full["current_rank"]
    if _is_blank(player_data.get('highest_rank')) and not _is_blank(full.get("high_rank")):
        player_data['highest_rank'] = full["high_rank"]

    # skin breakdown — only if empty
    if not player_data.get('skin_breakdown'):
        player_data['skin_breakdown'] = full.get("skin_breakdown", {})

    # collector / achievement / location / last_login / countries / squad / hero_history
    if _is_blank(player_data.get('collector_tier')) and not _is_blank(full.get("collector_tier")):
        player_data['collector_tier'] = full["collector_tier"]
    try: cp = int(player_data.get('collector_point') or 0)
    except (ValueError, TypeError): cp = 0
    if cp == 0 and full.get("collector_point"):
        player_data['collector_point'] = full["collector_point"]

    try: ap = int(player_data.get('achievement_points') or 0)
    except (ValueError, TypeError): ap = 0
    if ap == 0 and full.get("achievement_points"):
        player_data['achievement_points'] = full["achievement_points"]

    if _is_blank(player_data.get('location')) and not _is_blank(full.get("location")):
        player_data['location'] = full["location"]
    if _is_blank(player_data.get('last_online')) and not _is_blank(full.get("last_login")):
        player_data['last_online'] = full["last_login"]
    if _is_blank(player_data.get('login_country')) and not _is_blank(full.get("login_country")):
        player_data['login_country'] = full["login_country"]
    if _is_blank(player_data.get('reg_country')) and not _is_blank(full.get("reg_country")):
        player_data['reg_country'] = full["reg_country"]
    if _is_blank(player_data.get('squad')) and not _is_blank(full.get("squad")):
        player_data['squad'] = full["squad"]
    if _is_blank(player_data.get('hero_history')) and not _is_blank(full.get("hero_history")):
        player_data['hero_history'] = full["hero_history"]

    return player_data

# ────────────────────────────────────────────────────────────────
# SKIN BREAKDOWN (from raw protocol, fallback if API fails)
# ────────────────────────────────────────────────────────────────

def parse_skin_breakdown_raw(slist: list) -> Dict[str, int]:
...
def _dash(n=60): return "─" * n
def _eq(n=60):   return "=" * n


def format_hit_card(device_id: str, account_id, zone_id, player_data: dict) -> str:
    """Same layout as checker.py's 8-REQ card."""
    ...
    collector_point   = player_data.get('collector_point', 0) or 0
    achievement_point = player_data.get('achievement_points', 0) or 0
    ...
    lines = [
        ...
        f"COLL POINTS  : {collector_point:,}",
        f"ACHIEVEMENT  : {achievement_point}",
        _dash(),
        ...
        while True:
            ...
            for attempt in range(3):
                try:
                    ...
                    # Send as plain text (no parse_mode) — matches sample exactly
                    fut = asyncio.run_coroutine_threadsafe(
    ...
    if awaiting_single and auth:
        ...
        try:
            with GameConnection(device_id=device_id) as conn:
                ...
                # Fallback: fill N/A fields from lookup API
                try:
