#!/usr/bin/env python3
# ===================================================================
# PREMIUM DEVID SEKER - TELEGRAM BOT v3.5
# Account info and ban check are now SEPARATE commands
# ===================================================================
...
# ────────────────────────────────────────────────────────────────
# ACCOUNT INFO CHECK  (NO BAN CHECK)
# ────────────────────────────────────────────────────────────────

def process_account_info(device_id: str, account_id: int, zone_id: int, out_dir: str = None) -> Tuple[bool, Optional[dict]]:
    """Fetch account info only — no ban check."""
    conn = None
    try:
        ...
        log.error(f"process_account_info error: {e}")
        return False, None
    finally:
        if conn:
            try: conn.cleanup()
...
# ────────────────────────────────────────────────────────────────
# BAN CHECK  (STANDALONE)
# ────────────────────────────────────────────────────────────────

def process_ban_check(device_id: str, out_dir: str = None) -> Tuple[bool, Optional[dict]]:
    """Standalone ban check — logs in, checks ban, returns ban result only."""
    conn = None
    try:
        conn = GameConnection(device_id=device_id)
        conn.connect()
        ...
        if not conn.login_to_login_server():
            return False, {
                'device_id': device_id,
                'ban_status': 'LOGIN_FAILED',
                'ban_end': 'N/A',
                'reason': 'Failed to login to login server',
                'account_id': None,
                'zone_id': None,
            }
        ...
        # First check: login server ban flag
        ban_from_login = conn.ban_status
        ban_end_ts     = conn.ban_end_ts

        # Try to reach game server for deeper ban info
        ban_from_game = None
        if conn.get_game_server() and conn.connect_to_game_server():
            ban_from_game = conn.check_ban_status()

        # Decide which is real
        final_ban = "Not Banned"
        final_end = "N/A"

        if ban_from_login and ban_from_login != "NORMAL":
            final_ban = ban_from_login
            if ban_end_ts:
                final_end = fmt_ts(ban_end_ts)

        if ban_from_game and ban_from_game != "NORMAL":
            final_ban = ban_from_game
        ...
        result = {
            ...
            'ban_end': final_end,
            'is_banned': 'ban' in str(final_ban).lower() or 'suspend' in str(final_ban).lower() or 'restrict' in str(final_ban).lower(),
        }

        # Save result
        base = out_dir if out_dir else OUTPUT_DIR
        ban_dir = os.path.join(base, FOLDERS["ban_check"])
        os.makedirs(ban_dir, exist_ok=True)

        if result['is_banned']:
            banned_file = os.path.join(ban_dir, "banned.json")
        else:
            banned_file = os.path.join(ban_dir, "not_banned.json")
        ...
        log.error(f"process_ban_check error: {e}")
        return False, {
            'device_id': device_id,
            'ban_status': 'ERROR',
            'reason': str(e),
...
def kb_main(admin: bool = False):
    rows = [
        ...
        [InlineKeyboardButton("🚫 Ban Check (Single)", callback_data="tool_ban")],
...
def run_bulk_job(job: dict, loop, app):
    ...
    def worker(device_id: str):
        ...
        success, pd = process_account_info(device_id, acc, zone, session_dir)
        with job_lock:
            stats["checked"] += 1
            job["checked"] = stats["checked"]
...
async def cmd_help(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        f"🔍 *Single Check* – send one device ID as text (account info)\n"
        f"🚫 *Ban Check* – send one device ID as text (ban status only)\n"
...
# ────────────────────────────────────────────────────────────────
# TEXT HANDLER (SINGLE CHECK + BAN CHECK)
# ────────────────────────────────────────────────────────────────
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with job_lock:
        mode = None
        if active_jobs.get(uid, {}).get("awaiting_single_device"):
            mode = "single"
        elif active_jobs.get(uid, {}).get("awaiting_ban_device"):
            mode = "ban"

    if not (mode and auth):
        return

    device_id = update.message.text.strip()
    with job_lock:
        active_jobs.get(uid, {}).pop("awaiting_single_device", None)
        active_jobs.get(uid, {}).pop("awaiting_ban_device", None)
    ...
    if mode == "ban":
        ...
        ok, result = process_ban_check(device_id, OUTPUT_DIR)
        if not ok or not result:
            await update.message.reply_text("❌ Ban check failed."); return

        status = result.get('ban_status', 'Unknown')
        is_banned = result.get('is_banned', False)

        icon = "🔴" if is_banned else "🟢"
        text = (
            f"{icon} *BAN CHECK RESULT*\n"
            f"{'═'*30}\n\n"
            f"📱 Device  : `{device_id}`\n"
            f"🆔 Account : `{result.get('account_id', 'N/A')}`\n"
            f"🌐 Zone    : `{result.get('zone_id', 'N/A')}`\n\n"
            f"📋 Status  : *{status}*\n"
            f"⏳ Ends    : `{result.get('ban_end', 'N/A')}`\n"
            f"{'═'*30}\n"
            f"✨ *PREMIUM DEVID SEKER*"
        )
        await update.message.reply_text(text, parse_mode="Markdown")
        ...
        await user_manager.update_stats(uid, checked=1, hits=(1 if is_banned else 0))
        return
    ...
    success, pd = process_account_info(device_id, acc, zone, OUTPUT_DIR)
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "tool_ban":
        ...
        with job_lock:
            if uid not in active_jobs: active_jobs[uid] = {}
            active_jobs[uid]["awaiting_ban_device"] = True
        await query.edit_message_text(
            "🚫 *Ban Check*\nSend the Device ID as a text message.\n\n"
            "Returns ban status only — no account info.",
            parse_mode="Markdown"
        ); return
    ...
    if data == "menu_help":
        await query.edit_message_text(
            ...
            f"🔍 Single Check – type one Device ID (account info)\n"
            f"🚫 Ban Check – type one Device ID (ban status only)\n"
...
async def post_init(app: Application) -> None:
    ...
    log.info(f"🚀 {BOT_NAME} v{BOT_VERSION} online — 10 threads, ban check separated")
