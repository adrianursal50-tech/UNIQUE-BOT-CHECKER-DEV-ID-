#!/usr/bin/env python3
# ===================================================================
# DEV ID Checker — Telegram Bot v4.2  ◈  SHIN CYBER EDITION
# -------------------------------------------------------------------
#   • Single + Bulk check (real TCP validation)
#   • Real Validator (TCP login, live stats, 30 threads)
#   • Generator (and_ / ios_ mixed, unique)
#   • Guest-account (221/222) rejection at every stage
...
def is_guest_account(acc) -> bool:
    """Guest/unregistered accounts have IDs starting with 221 or 222."""
...
def line_success(did, pd):
    ...
    if "banned" in ban.lower() or "suspend" in ban.lower():
        base += f"  ⚠️ `{ban[:18]}`"
    return base
...
def extract_player_data(result):
    ...
    # Safety net — reject guest accounts before parsing ban info
...
def process_detail(device_id, account_id, zone_id, want_debug=False):
    ...
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            if not result:
                # Try ban check — if banned, classify correctly
                try:
                    bs = conn.check_ban_status()
                    if "banned" in str(bs).lower() or "suspend" in str(bs).lower():
                        return False, None, f"banned:{bs}", debug_bundle
                except: pass
            ...
            player_data = {
                ...
                "ban_status": ban_stat if ("banned" in str(ban_stat).lower() or "suspend" in str(ban_stat).lower()) else extracted.get("ban_status"),
                "v2l_status": v2l,
...
def save_account_v2(account_info, player_data, out_dir=None):
    ...
    # Hard reject guest accounts — never write anywhere
    if is_guest_account(acc):
        return None
    ...
    is_banned = ("banned" in bs_low) or ("suspend" in bs_low) or ("restrict" in bs_low)
    if is_banned:
...
def write_session_card(session_dir, device, acc, zone, pd, is_banned):
    try:
        ...
        if is_banned:
            bs_low = str(pd.get("ban_status","")).lower()
...
def run_bulk_job(job, loop, app):
    ...
    def worker(did):
        ...
        stats.inc("checked")
        if ok and pd:
            # Additional guest safety net
            if is_guest_account(pd.get('account_id')):
                stats.inc("failed"); upd(); return
            is_b = ('banned' in str(pd.get('ban_status','')).lower() or
                    'suspend' in str(pd.get('ban_status','')).lower())
...
async def post_init(app):
    ...
    log.info(f"🚀 {BOT_NAME} v{BOT_VERSION} online — guest-account filter active")
...
def main():
    ...
    log.info("🔥 Starting SHIN Cyber Edition v4.2...")
    try:
