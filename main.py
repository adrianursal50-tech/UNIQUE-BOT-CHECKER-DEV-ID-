...
KICK_THREADS   = int(os.environ.get("KICK_THREADS", "5"))
KICK_DELAY     = float(os.environ.get("KICK_DELAY", "3.0"))     # 3s per attempt
KICK_DEFAULT_ROUNDS = int(os.environ.get("KICK_ROUNDS", "0"))   # 0 = unlimited
...
# ============== GLOBAL KICK REGISTRY ==============
# user_id -> {device_id, rounds, stats, stop_event, started, chat_id, message_id, done}
kick_jobs: Dict[int, Dict[str, Any]] = {}
kick_lock = threading.Lock()

def kick_status_text(uid: int) -> str:
    with kick_lock:
        job = kick_jobs.get(uid)
    if not job:
        return "📭 <b>No active kick job.</b>\n<i>Use /kick &lt;device_id&gt; to start.</i>"
    s = job["stats"]
    elapsed = time.time() - job["started"]
    rounds_txt = "∞" if job["rounds"] == 0 else str(job["rounds"])
    pct = (s["attempts"] / job["rounds"] * 100) if job["rounds"] > 0 else 0
    bar_w = 20
    filled = int(bar_w * pct / 100) if pct else 0
    bar = "▓" * filled + "░" * (bar_w - filled)
    status = "🟢 RUNNING" if not job.get("done") else "✅ FINISHED"
    if job.get("stop_event") and job["stop_event"].is_set() and not job.get("done"):
        status = "🛑 STOPPING"
    return (
        ...
        f"\n<i>Use /kickstop to cancel · /kickstatus to refresh</i>"
    )
...
def render_card(did: str, p: Dict) -> str:
    acc=p.get("player_id","?"); zone=p.get("server","?"); nick=p.get("nickname","N/A")
    level=p.get("level","N/A"); heroes=p.get("hero_count","N/A"); skins=p.get("skin_count","N/A")
    cur=p.get("current_rank") or "Unranked"; high=p.get("high_rank") or "N/A"
    coll=p.get("collector_tier") or "N/A"
    cpt=p.get("collector_point"); cpt_s=f"{int(cpt):,}" if isinstance(cpt,(int,float)) and cpt>0 else "N/A"
    loc=p.get("location") or "NOT FOUND"; ll=p.get("last_login") or "N/A"
    squad=p.get("squad") or "N/A"; wr=p.get("win_rate","N/A")
    hh=p.get("hero_history") or []; hh_s=", ".join(str(x) for x in hh[:5]) if hh else "N/A"
    found=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    return (
        f"◇  <b>DEVICE ID CHECK RESULT</b>  ◇\n{LINE_HEAVY}\n"
        f"┣ 🆔 Device : <code>{html.escape(did)}</code>\n"
        f"┣ 🎮 Acc    : <code>{_esc(acc)}</code>\n"
        f"┣ 🌐 Zone   : <code>{_esc(zone)}</code>\n"
        f"┣ 🏷 Nick   : <b>{_esc(nick)}</b>\n"
        f"┣ 📈 Level  : <code>{_esc(level)}</code>\n"
        f"┣ 🦸 Heroes : <code>{_esc(heroes)}</code>\n"
        f"┗ 🎨 Skins  : <code>{_esc(skins)}</code>\n\n"
        f"◇  <b>RANKS</b>  ◇\n{LINE_HEAVY}\n"
        ...
        f"◇  <b>ACCOUNT</b>  ◇\n{LINE_HEAVY}\n"
        f"┣ 📍 Loc    : <code>{_esc(loc)}</code>\n"
        f"┣ ⏰ Login  : <code>{_esc(ll)}</code>\n"
        f"┣ 👥 Squad  : <code>{_esc(squad)}</code>\n"
        f"┣ 🎯 WR     : <code>{_esc(wr)}</code>\n"
...
def render_job(job: Job) -> str:
    bar = progress_bar(job.pct, 20); eta = ""
    ...
    title = {"ban": "BAN SCAN", "kick": "KICK SCAN"}.get(job.mode, "SCANNING")
    return (f"⚡ <b>{title}</b> — <i>{html.escape(job.input_name)}</i>\n"
        ...
        async with sem:
            ...
            if res.get("status") == "success":
                ...
                if lvl < 9:
                    ...
                    job.valid += 1; job.results.append(save_line(did, p))
    ...
    async def one(did):
        ...
        async with sem:
            ...
            if res.get("status") == "success":
                ...
                job.invalid += 1
                job.results.append(f"{did} | CHECK FAILED | {res.get('error','unknown')}")
                job.latest.append(fmt_line(False, did, str(res.get("error","unknown"))[:60]))
            now = time.time()
            if now - job.last_update >= UPDATE_INTERVAL:
                job.last_update = now
        ...
        if job.results:
            ...
            try:
                await bot.send_document(chat_id=chat_id,
                    ...
                    caption=(f"✅ <b>Ban scan done</b>\n🚫 Banned: <b>{job.banned}</b> │ "
                             f"✅ Clean: <b>{job.valid}</b> │ ❌ Failed: <b>{job.invalid}</b>\n"
                             f"⏱ {job.elapsed:.1f}s"), parse_mode=ParseMode.HTML)
            except Exception as e: log.warning("send_document failed: %s", e)
...
async def cmd_start(msg: Message, command: CommandObject):
    ...
    if is_admin:
        txt += ("👑 <b>Admin</b>\n"
                "• <code>/kick &lt;id&gt;</code> — kick target\n"
                "• <code>/kickstatus</code> — kick progress\n"
                "• <code>/kickstop</code> — stop kick\n")
...
async def cmd_help(msg: Message):
    ...
    if storage.is_admin(msg.from_user.id):
        txt += (f"👑 <code>/kick and_xxxx…</code> — kick target ({KICK_DELAY}s delay/attempt)\n"
                "👑 <code>/kickstatus</code> — kick progress\n"
                "👑 <code>/kickstop</code> — stop running kick\n")
    txt += "📊 <code>/status</code> · 🛑 <code>/stop</code>"
...
async def cmd_kick(msg: Message, command: CommandObject):
    ...
    # Refuse if a kick already running for this user
    with kick_lock:
        existing = kick_jobs.get(uid)
    if existing and not existing.get("done"):
        return await msg.answer("⚠️ You already have a kick running. Use /kickstatus or /kickstop.")

    rounds = KICK_DEFAULT_ROUNDS  # 0 = unlimited
    ...
    with kick_lock:
        kick_jobs[uid]["message_id"] = ph.message_id

    # Progress callback updates the shared stats
    def _on_prog(s):
        with kick_lock:
            job = kick_jobs.get(uid)
            if job: job["stats"] = s
    ...
    def _runner():
        try:
            kick_device(did, rounds=rounds, delay=KICK_DELAY,
                        stop_event=stop_event, on_progress=_on_prog)
        finally:
            with kick_lock:
                j = kick_jobs.get(uid)
                if j: j["done"] = True
    ...
    # Periodically refresh the message while running
        ...
        while True:
            ...
            try:
                await bot.edit_message_text(
                    chat_id=ph.chat.id, message_id=ph.message_id,
                    text=kick_status_text(uid),
                    reply_markup=kick_buttons(uid))
            except Exception: pass
            if done:
                # final refresh, then cleanup after 60s
                await asyncio.sleep(60)
                with kick_lock:
                    kick_jobs.pop(uid, None)
                return
...
async def cmd_kickstop(msg: Message):
    ...
    if not job or job.get("done"):
        return await msg.answer("📭 No active kick job.")
    job["stop_event"].set()
    await msg.answer("🛑 Stop signal sent — kick will end after current attempt.")
...
async def on_document(msg: Message):
    ...
    if mode == "ban":
        asyncio.create_task(run_bulk_ban(bot, msg.chat.id, ph.message_id,
                                          msg.from_user.id, ids, doc.file_name))
    else:
        asyncio.create_task(run_bulk_player(bot, msg.chat.id, ph.message_id,
                                             msg.from_user.id, ids, doc.file_name))
...
async def cb_ui_kick(cq):
    if not storage.is_admin(cq.from_user.id): return await cq.answer("Admin only", show_alert=True)
    await cq.message.answer(f"Use <code>/kick &lt;device_id&gt;</code> · delay {KICK_DELAY}s")
    await cq.answer()

@router.callback_query(F.data == "ui:kickstatus")
async def cb_ui_kickstatus(cq):
    if not storage.is_admin(cq.from_user.id): return await cq.answer("Admin only", show_alert=True)
    await cq.message.answer(kick_status_text(cq.from_user.id), reply_markup=kick_buttons(cq.from_user.id))
    await cq.answer()

# Kick control callbacks
...
async def cb_kick_refresh(cq):
    ...
    if target != cq.from_user.id and not storage.is_admin(cq.from_user.id):
        return await cq.answer("Not allowed", show_alert=True)
    try:
        await cq.message.edit_text(kick_status_text(target), reply_markup=kick_buttons(target))
    except Exception: pass
...
async def cb_kick_stop(cq):
    ...
    with kick_lock:
        job = kick_jobs.get(target)
    if not job or job.get("done"):
        return await cq.answer("No active kick", show_alert=True)
    job["stop_event"].set()
...
async def set_commands():
    cmds = [BotCommand(command="start",  description="Start"),
            ...
            BotCommand(command="kick",      description="Kick target (admin)"),
...
async def main():
    await set_commands()
    log.info("Bot started. Threads=%d · Kick delay=%.1fs", BULK_THREADS, KICK_DELAY)
