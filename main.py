#!/usr/bin/env python3
# ===================================================================
# DEV ID Checker — Telegram Bot v4.3  ◈  SHIN CYBER EDITION
# -------------------------------------------------------------------
#   • FIXED: substring bug — "Not Banned" was matching "banned"
#   • FIXED: banned / hits separation is now strict
...
def is_banned_status(bs) -> bool:
    """
    Strict ban check — 'Not Banned' must NOT match 'banned'.
    Only returns True when the status clearly indicates a ban.
    """
    ...
    if s == "not banned": return False
    # Only positive ban markers
...
def extract_player_data(result):
    ...
    if is_guest_account(pd.get(0)):
        ...
        return None
    try:
        # Ban code parsing — ONLY accept 1-5 as real bans
...
def process_detail(device_id, account_id, zone_id, want_debug=False):
    ...
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            # Determine final ban status — prefer the stricter of the two
...
def write_session_card(session_dir, device, acc, zone, pd, is_banned):
    try:
        ...
        # Double-check the ban status — never write to Banned unless truly banned
        real_ban = is_banned_status(pd.get("ban_status", ""))
        # Override caller if caller said banned but status says not
...
async def post_init(app):
    ...
    log.info(f"🚀 {BOT_NAME} v{BOT_VERSION} online — strict ban classifier active")
...
def main():
    ...
    log.info("🔥 Starting SHIN Cyber Edition v4.3...")
    try:
