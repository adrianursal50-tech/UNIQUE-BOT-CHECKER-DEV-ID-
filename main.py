#!/usr/bin/env python3
# ===================================================================
# DEV ID Checker — Telegram Bot v3.6  ◈  SHIN CYBER EDITION
# -------------------------------------------------------------------
#   • Single check + Bulk check (device IDs)
#   • Login:Pass → Device ID converter
#   • Channel gate, admin panel, key system
#   • Level/Skin bracket file splitting
#   • Watermark: @SHINRT771
# ===================================================================
...
def combo_to_device_id(combo: str, platform: str = "and") -> Optional[str]:
    combo = (combo or "").strip()
    if not combo or ":" not in combo: return None
    platform = platform.lower().strip()
    if platform not in ("and", "ios"): platform = "and"
    b = combo.encode("utf-8")
    imei_md5 = hashlib.md5(b).hexdigest()
    h = hashlib.sha256(b).hexdigest()
    android_id = h[:16]
    adv = hashlib.md5(b + b"_adid").hexdigest()
    advertising_id = f"{adv[:8]}-{adv[8:12]}-{adv[12:16]}-{adv[16:20]}-{adv[20:32]}"
    return f"{platform}_{imei_md5}{android_id}{advertising_id}"

def looks_like_combo(s: str) -> bool:
    if not isinstance(s, str): return False
    s = s.strip()
    if not s or " " in s or ":" not in s: return False
    if s.count(":") < 1 or s.count(":") > 4: return False
    left, right = s.split(":", 1)
    if len(left) < 2 or len(right) < 1: return False
    if len(s) > 200: return False
    if "@" in left: return True
    if re.match(r"^[A-Za-z0-9._\-]{3,}$", left): return True
    return False
...
def render_welcome(name: str, is_admin: bool, expiry: str, checked: int, hits: int) -> str:
    ...
    return (
        ...
        f"┣ 🔄 Convert → `login:pass` → device ID\n"
...
def render_convert_menu(platform: str = "and") -> str:
    p_icon = "🤖" if platform == "and" else "🍎"
    return (
        f"{banner_line()}\n{LINE_HEAVY}\n"
        f"◇  *LOGIN:PASS → DEVICE ID*  ◇\n"
        f"{LINE_HEAVY}\n"
        f"┣ {p_icon} Platform : `{platform.upper()}`\n"
        f"┣ 📝 Format   : `login:password`\n"
...
def render_convert_result(count: int, results: list, platform: str, preview: int = 8) -> str:
    p_icon = "🤖" if platform == "and" else "🍎"
    lines = [
        f"✅  *CONVERTED {count} COMBO(S)*",
        LINE_HEAVY,
        f"┣ {p_icon} Platform : `{platform.upper()}`",
        f"┗ 📄 Total    : `{count}`",
        LINE_HEAVY, "",
    ]
    for i, (combo, did) in enumerate(results[:preview]):
        safe_combo = str(combo).replace("`", "'")[:40]
        lines.append(f"`{i+1}.` `{safe_combo}`")
        lines.append(f"     ↳ `{did}`")
        lines.append("")
    if len(results) > preview:
        lines.append(f"_… and {len(results) - preview} more in attached file_")
    lines.append(LINE_HEAVY)
    lines.append(f"👑 {BRAND}")
    return "\n".join(lines)

def kb_convert_platform():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🤖  Android (and_)", callback_data="conv_set:and"),
         InlineKeyboardButton("🍎  iOS (ios_)",     callback_data="conv_set:ios")],
        [InlineKeyboardButton("⬅️  Back", callback_data="conv_back")],
    ])
    ...
    def _coerce_zone(z):
        ...
        if isinstance(z, dict):
            for k in (0, 1, "0", "id", "zone", "zone_id"):
                if k in z:
                    return GameConnection._coerce_zone(z[k])
            if z:
                return GameConnection._coerce_zone(next(iter(z.values())))
        try: return int(z)
    ...
    def login_to_login_server(self):
        ...
        for _ in range(4):
            ...
            if pid == 2 and res:
                ...
                log.info(f"[LOGIN] acc={self.account_id} zone={self.zone_id} sess={str(self.session_key)[:12]}...")
                return True
...
def main_menu_kb(include_admin: bool = False):
    rows = [
        ...
        [InlineKeyboardButton("🔄  Convert Login:Pass", callback_data="tool_convert")],
...
async def cmd_convert(update, ctx):
    uid = update.effective_user.id
    await user_manager.register_user(uid, update.effective_user.username, update.effective_user.first_name)
    if not await gate(update, ctx): return
    ...
    await update.message.reply_text(render_convert_menu("and"), parse_mode="Markdown",
        reply_markup=kb_convert_platform())
...
async def cmd_help(update, ctx):
    ...
    await update.message.reply_text(
        ...
        "┣ 🔄 Convert — send `login:pass` combos\n"
        "┣ 📊 Statistics — rank counters\n"
        "┗ 📖 Menu — /menu\n\n"
...
# ════════════════════════════════════════════════════════════════
# CONVERT INPUT PROCESSOR
# ════════════════════════════════════════════════════════════════

async def _process_convert_input(update, ctx, text_or_data, platform):
    ...
    combos = [l for l in lines if looks_like_combo(l)]
    ...
    truncated = False
    if len(combos) > MAX_CONVERT_BULK:
        combos = combos[:MAX_CONVERT_BULK]
        truncated = True

    results = []
    for c in combos:
        did = combo_to_device_id(c, platform)
        if did: results.append((c, did))

    if not results:
        await update.effective_message.reply_text("❌ Could not convert any combos.")
        return

    if len(results) <= 15:
        txt = render_convert_result(len(results), results, platform, preview=15)
        if truncated:
            txt += f"\n\n⚠️ _Truncated to first {MAX_CONVERT_BULK}_"
        await update.effective_message.reply_text(txt, parse_mode="Markdown")
        return

    txt = render_convert_result(len(results), results, platform, preview=8)
    if truncated:
        txt += f"\n\n⚠️ _Truncated to first {MAX_CONVERT_BULK}_"
    await update.effective_message.reply_text(txt, parse_mode="Markdown")
    ...
    try:
        await update.effective_message.reply_document(
            document=InputFile(bio, filename=f"devid_{platform}_{ts}.txt"),
            caption=f"📄  *{len(results)} device IDs* — platform: `{platform}`",
            parse_mode="Markdown")
    except Exception as e:
        log.error(f"convert file send: {e}")
...
async def handle_document(update, ctx):
    ...
    with job_lock:
        ...
        wants_convert = job.get("awaiting_convert_input")
        conv_platform = job.get("convert_platform", "and")
    ...
    if wants_convert:
        await _process_convert_input(update, ctx, bytes(data), conv_platform)
        with job_lock:
            active_jobs.get(uid, {}).pop("awaiting_convert_input", None)
        return

    if not wants_bulk:
        await update.message.reply_text(
            "Use /start → 🔥 *Bulk Check* first, or /convert for combos.",
            parse_mode="Markdown"); return
...
async def on_text(update, ctx):
    ...
    with job_lock:
        ...
        awaiting_convert = job.get("awaiting_convert_input")
        conv_platform = job.get("convert_platform", "and")

    # ── Convert flow ──
    if awaiting_convert and auth:
        with job_lock:
            active_jobs.get(uid, {}).pop("awaiting_convert_input", None)
        await _process_convert_input(update, ctx, text, conv_platform)
        return
    ...
    # ── Auto-detect combo ──
    if auth and "\n" not in text and looks_like_combo(text.strip()):
        did = combo_to_device_id(text.strip(), "and")
        if did:
            await update.message.reply_text(
                f"🔄  *AUTO-CONVERTED*\n{LINE_HEAVY}\n"
                f"`{text.strip()}`\n     ↳ `{did}`\n\n"
                f"_Tap 🔄 Convert for bulk / iOS_",
                parse_mode="Markdown")
            return
...
async def handle_callback(update, ctx):
    ...
    if data == "tool_single":
        ...
        await q.edit_message_text(
            ...
            f"{LINE_HEAVY}\n👑 {BRAND}", parse_mode="Markdown"); return
    if data == "tool_convert":
        auth, r = await user_manager.is_authorized(uid)
        if not auth:
            await q.edit_message_text(f"🚫 `{r}`", parse_mode="Markdown"); return
        await q.edit_message_text(
            render_convert_menu("and"),
            parse_mode="Markdown",
            reply_markup=kb_convert_platform()
        ); return
    if data == "conv_back":
        ...
        with job_lock:
            active_jobs.get(uid, {}).pop("awaiting_convert_input", None)
            active_jobs.get(uid, {}).pop("convert_platform", None)
        await q.edit_message_text(
            render_menu(),
            parse_mode="Markdown",
            reply_markup=main_menu_kb(include_admin=(uid == OWNER_ID))
        ); return
    if data.startswith("conv_set:"):
        auth, r = await user_manager.is_authorized(uid)
        if not auth:
            await q.edit_message_text(f"🚫 `{r}`", parse_mode="Markdown"); return
        ...
        with job_lock:
            if uid not in active_jobs: active_jobs[uid] = {}
            active_jobs[uid]["awaiting_convert_input"] = True
            active_jobs[uid]["convert_platform"] = platform
        await q.edit_message_text(
            render_convert_menu(platform),
            parse_mode="Markdown"
        ); return
    if data == "tool_stats":
        with COUNTER_LOCK:
...
async def post_init(app):
    cmds = [
        ...
        BotCommand("convert", "Login:Pass → Device ID"),
...
def main():
    ...
    log.info("🔥 Starting SHIN Cyber Edition v3.6...")
    try:
