...
# Full card output (checker.py style) + patient lookup API fallback
# ===================================================================
...
# ════════════════════════════════════════════════════════════════
# LOOKUP API — fills N/A fields, same normalization as checker.py
# ════════════════════════════════════════════════════════════════
        ...
        try:
            ...
            if resp.status_code == 429:
                ...
                if attempt < LOOKUP_RETRIES and total_waited + wait <= LOOKUP_MAX_TOTAL_WAIT:
                    _lookup_log(f"  429 -> sleeping {wait:.1f}s "
                                f"(attempt {attempt+1}/{LOOKUP_RETRIES+1}, "
...
def _normalize_player_data(data: Dict[str, Any]) -> Dict[str, Any]:
    """Same shape as checker.py's FullInfoLookup."""
...
def fetch_full_info(account_id, zone_id) -> Dict[str, Any]:
    """Fetch + normalize. Never fabricates values."""
...
def apply_lookup_fallback(player_data: dict, full: Dict[str, Any]) -> dict:
    """
    Fill fields that are N/A / empty / 0 using real API values.
    Never overwrites an existing value. Never invents data.
    """
    ...
    # nickname (also replaces "Player_XXX" placeholders)
...
# ════════════════════════════════════════════════════════════════
# CARD FORMAT — same layout as the sample the user posted
# ════════════════════════════════════════════════════════════════
