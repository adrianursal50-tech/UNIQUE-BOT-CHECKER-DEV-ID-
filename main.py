...
# Added: full lookup (location / last_login / collector_tier / etc.)
# ===================================================================
...
# ────────────────────────────────────────────────────────────────
# FULL INFO LOOKUP  (location / last_login / collector_tier / etc.)
# ────────────────────────────────────────────────────────────────
...
def fetch_full_info(account_id, zone_id) -> Dict[str, Any]:
    """Returns normalized dict with location / last_login / collector_tier / etc."""
...
def format_account_card(device: str, acc: int, zone: int, player_data: dict) -> str:
    ...
    lines = [
        ...
        f"Collector Tier : {player_data.get('collector_tier','N/A')} ({player_data.get('collector_point',0)} pts)",
        f"Achievement    : {player_data.get('achievement_points',0)}",
        f"Squad          : {player_data.get('squad','—')}",
        f"Bindings       : {player_data.get('bindings','—')}",
        f"Win Rate       : {player_data.get('win_rate',0)}%  ({player_data.get('matches',0)} matches, {player_data.get('mvp',0)} MVP)",
        "─" * 60,
        f"Skins → Supreme:{sb.get('Supreme',0)}  Grand:{sb.get('Grand',0)}  "
        f"Exquisite:{sb.get('Exquisite',0)}  Deluxe:{sb.get('Deluxe',0)}  "
        f"Exceptional:{sb.get('Exceptional',0)}  Common:{sb.get('Common',0)}",
        "═" * 60,
    ]
    return "\n".join(lines)
...
def process_detail(device_id: str, account_id: int, zone_id: int, out_dir: str = None) -> Tuple[bool, Optional[dict]]:
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            # ── NEW: pull location / last_login / collector_tier from lookup API ──
            full = fetch_full_info(account_id, zone_id)
            if full.get("status") == "success":
                nick      = full.get("nickname")     or nick
                level     = full.get("level")        or level
                skin_cnt  = full.get("skin_count")   or skin_cnt
                hero_cnt  = full.get("hero_count")   or hero_cnt
                cur_rank  = full.get("current_rank") or map_rank(cur_rank_val)
                high_rank = full.get("high_rank")    or (map_rank(max_rank_val) if max_rank_val else cur_rank)
                location  = full.get("location", "NOT FOUND")
                last_login= full.get("last_login", "N/A")
                collector_tier   = full.get("collector_tier", "N/A")
                collector_point  = full.get("collector_point", 0)
                achievement_pts  = full.get("achievement_points", 0)
                squad     = full.get("squad", "—")
                bindings  = full.get("bindings", "—")
                win_rate  = full.get("win_rate", 0)
                matches   = full.get("matches", 0)
                mvp       = full.get("mvp", 0)
                skin_breakdown = full.get("skin_breakdown", {})
                lookup_ok = True
            else:
            ...
            player_data = {
                ...
                'lookup_status': "success" if lookup_ok else "error",
                'device_id':     device_id,
    ...
    if awaiting_single and auth:
        ...
        try:
            with GameConnection(device_id=device_id) as conn:
                ...
                # ── NEW: lookup API for location / last_login / collector_tier ──
                ...
                if full.get("status") == "success":
                    ...
                    skin_cnt    = full.get("skin_count")   or skin_cnt
                    hero_cnt    = full.get("hero_count")   or hero_cnt
                    cur_rank    = full.get("current_rank") or cur_rank
                    highest_rank= full.get("high_rank")    or highest_rank
                    location    = full.get("location", "NOT FOUND")
                    last_login  = full.get("last_login", "N/A")
                    coll_tier   = full.get("collector_tier", "N/A")
                    coll_pt     = full.get("collector_point", 0)
                    achievement = full.get("achievement_points", 0)
                    squad       = full.get("squad", "—")
                    win_rate    = full.get("win_rate", 0)
                    matches     = full.get("matches", 0)
                    mvp         = full.get("mvp", 0)
                ...
                player_data = {
                    ...
                    'current_rank': cur_rank, 'highest_rank': highest_rank,
                    'ban_status': ban_stat, 'v2l_status': v2l,
                    'created_at': created_at, 'last_online': last_login,
                    'location': location, 'collector_tier': coll_tier,
                    'collector_point': coll_pt, 'achievement_points': achievement,
                    'squad': squad, 'win_rate': win_rate,
                    'matches': matches, 'mvp': mvp,
                    'skin_breakdown': (full.get('skin_breakdown', {}) if full.get("status") == "success"
                                       else parse_skin_breakdown(skin_info.get(92, []))),
                }
                save_account(
                ...
                details = (
                    ...
                    f"🛡 Squad        : {squad}\n"
                    f"📈 Win Rate     : {win_rate}% \\({matches} matches, {mvp} MVP\\)\n"
