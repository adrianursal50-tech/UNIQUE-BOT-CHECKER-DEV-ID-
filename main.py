#!/usr/bin/env python3
# ===================================================================
# SHIN DevID Checker — Telegram Bot v2.4
# -------------------------------------------------------------------
#   • Channel gate: 3 required channels before use
#   • Admin panel: live running-session stats (valid/banned/invalid)
#   • Structured zip: Valid_Hits/  Banned/  summary.txt
#   • Lookup API integration: last_login, location, collector, squad
#   • Watermark: @SHINRT771
# ===================================================================
...
# ────────────────────────────────────────────────────────────────
# LOOKUP API — fetches full profile (last_login, location, collector, squad)
# ────────────────────────────────────────────────────────────────
...
def _merge_lookup_into_player_data(pd: dict, api_pd: dict) -> dict:
    ...
    for key, conv in [
        ...
        ("collector_tier",     _s),
        ("collector_point",    _i),
        ("achievement_points", _i),
        ("matches",            _i),
        ("mvp",                _i),
        ("win_rate",           _i),
    ]:
        val = conv(api_pd.get(key))
        if val is not None:
            pd[key] = val

    sb = api_pd.get("skin_breakdown")
    if isinstance(sb, dict) and sb:
        pd["skin_breakdown"] = {
            "supreme":     _i(sb.get("Supreme"), 0),
            "grand":       _i(sb.get("Grand"), 0),
            "exquisite":   _i(sb.get("Exquisite"), 0),
            "deluxe":      _i(sb.get("Deluxe"), 0),
            "exceptional": _i(sb.get("Exceptional"), 0),
            "common":      _i(sb.get("Common"), 0),
        }

    for key in ("hero_history", "top_heroes"):
        v = api_pd.get(key)
        if isinstance(v, list) and v:
            ...
            for item in v:
                if isinstance(item, str) and item.strip():
                    ...
                    for f in ("name", "hero_name", "nickname"):
                        if isinstance(item.get(f), str) and item[f].strip():
                            names.append(item[f].strip()); break
            if names:
                pd["hero_history"] = names[:5]
                break

    hr = _s(api_pd.get("high_rank"))
    if hr and not pd.get("highest_rank"):
        pd["highest_rank"] = hr
...
def render_join_gate(first_name: str = "there") -> str:
    lines = [
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━━━",
        "",
        f"Hey *{first_name}*, before you can use the bot",
        "you must join all three required channels:",
        "",
    ]
    for ch in REQUIRED_CHANNELS:
...
def get_v2l_status(conn, role_id: int, zone_id: int) -> str:
    for pid_req, pid_resp_list in [
        (10208, [10208]), (10145, [10146, 10160]), (10143, [10144]),
    ]:
        try:
...
# ────────────────────────────────────────────────────────────────
# FIELD EXTRACTION HELPERS (fallbacks when API is down)
# ────────────────────────────────────────────────────────────────
...
def _looks_like_unix_ts(v) -> bool:
    ...
    v = int(v)
    return 1420070400 <= v <= 2051222400
...
def format_card_v2(device: str, data: dict, found_at: Optional[str] = None) -> str:
    ...
    collector     = data.get("collector_tier") or data.get("collector") or "N/A"
    coll_points   = data.get("collector_point") or data.get("collector_points")
    if isinstance(coll_points, (int, float)) and coll_points > 0:
        coll_points = f"{int(coll_points):,}"
    else:
        coll_points = "N/A"

    ach = data.get("achievement_points") or data.get("achievement")
    ...
    win_rate_str  = f"{win_rate}%" if isinstance(win_rate, (int, float)) and win_rate > 0 else "N/A"
    mvp           = data.get("mvp")
    mvp_str       = f"{int(mvp):,}" if isinstance(mvp, (int, float)) and mvp > 0 else "N/A"
...
def card_to_json(device: str, data: dict) -> dict:
    return {
        ...
        "collector":          data.get("collector_tier"),
        "collector_points":   data.get("collector_point"),
        "achievement":        data.get("achievement_points"),
...
def parse_skin_breakdown(slist: list) -> Dict[str, int]:
    tiers = {'supreme': 0, 'grand': 0, 'exquisite': 0, 'deluxe': 0,
             'exceptional': 0, 'common': 0}
...
# ────────────────────────────────────────────────────────────────
# SESSION CARD WRITER (structured zip folders)
# ────────────────────────────────────────────────────────────────
...
def write_session_card(session_dir: str, device: str, acc, zone,
                       player_data: dict, is_banned: bool) -> Optional[str]:
    try:
        ...
        if is_banned:
            ...
            low = str(nick).lower()
            if low in ("", "n/a", "guest", "unknown") or low.startswith("player_"):
                return None
        ...
        log.error(f"write_session_card failed: {e}")
        return None
...
def process_detail(device_id: str, account_id: int, zone_id: int,
    ...
    try:
        # 1. Lookup API — last_login / location / collector / squad / etc.
        api_pd = {}
        api_result = lookup_via_api(account_id, zone_id)
        if api_result.get("status") == "success":
            api_pd = api_result.get("player_data", {}) or {}
            log.info(f"Lookup API OK {account_id}:{zone_id} — "
                     f"collector={api_pd.get('collector_tier')} "
                     f"last_login={api_pd.get('last_login')}")
        else:
            log.warning(f"Lookup API failed {account_id}:{zone_id}: {api_result.get('error')}")

        # 2. In-game protocol (fill what API doesn't cover)
        with GameConnection(device_id=device_id) as conn:
            ...
            login_c, reg_c = _extract_countries(pd, skin_info, role_info)
            ...
            player_data = {
                ...
                'skin_count':       skin_cnt,
                'hero_count':       hero_cnt,
                'matches':          _extract_matches(pd, skin_info, role_info) or "N/A",
                ...
                'reg_country':      reg_c,
                # API-fillable placeholders
                'collector_tier':   None,
                'collector_point':  None,
            ...
            # 3. Merge API data (API wins)
            player_data = _merge_lookup_into_player_data(player_data, api_pd)

            # Fallbacks when API is down
            if not player_data.get('last_login'):
                ll = _extract_last_login(pd, skin_info, role_info)
                if ll: player_data['last_login'] = ll
            if not player_data.get('collector_tier'):
                ct, cp = _extract_collector_info(pd, skin_info, role_info)
                if ct: player_data['collector_tier'] = ct
                if cp and not player_data.get('collector_point'):
                    player_data['collector_point'] = cp
            if not player_data.get('squad'):
                sq = _extract_squad_name(pd, skin_info, role_info)
                if sq: player_data['squad'] = sq
            if not player_data.get('hero_history'):
                hh = _extract_hero_history(pd, skin_info, role_info)
                if hh: player_data['hero_history'] = hh
...
def render_welcome(first_name: str, is_admin: bool, expiry_display: str,
                   total_checked: int, total_hits: int) -> str:
    role_line = "👑  Admin — unlimited" if is_admin else f"🎟️  Access until: {expiry_display}"
    return (
        f"🎮  *DEV ID Checker Bot BY SHIN*\n"
        f"{LINE_HEAVY}\n"
        ...
        f"┗ 🎯 Hits    : `{total_hits}`\n\n"
        f"*How to use:*\n"
        f"• Tap 🔥 *Bulk Check* → send a `.txt` file\n"
...
def render_no_key() -> str:
    return (
        f"🔒  *ACCESS RESTRICTED*\n"
        f"{LINE_HEAVY}\n"
        f"Welcome to *DEV ID Checker Bot BY SHIN*.\n\n"
        ...
        f"Already have a key? Use `/redeem <key>`\n"
        f"{LINE_HEAVY}\n"
        f"👑 {BRAND}"
    )
...
def render_live(snap, status="RUNNING"):
    ...
    return (
        f"{icon}  *DEV ID Checker — Bulk Check*\n"
        f"{LINE_HEAVY}\n"
        f"`{_bar(pct)}` *{pct:5.1f}%*\n\n"
        f"🎯  *Hits*     : `{snap['hits']}`\n"
...
def run_bulk_job(job: dict, loop, app):
    ...
    try:
        with open(os.path.join(session_dir, "summary.txt"), "w", encoding="utf-8") as f:
            f.write(
                f"SHIN DevID Checker — Bulk Results\n"
                f"=================================\n"
    ...
    if awaiting_single and auth:
        ...
        if not ok or not pd:
            await msg.edit_text(
                ...
                f"Login OK: `{acc}:{zone}` — but no detail returned.",
                parse_mode="Markdown"
            ); return
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "verify_join":
        ...
        if ok:
            await query.edit_message_text(
                ...
                f"Thanks for joining. Tap /start to begin.",
                parse_mode="Markdown"
            )
        else:
            ...
            await query.edit_message_text(
                f"⚠️  *Still missing:*\n{miss_list}\n\n"
                f"Join all channels, then tap *Verify* again.",
                parse_mode="Markdown",
    ...
    if uid != OWNER_ID:
        ...
        if not ok:
            await query.edit_message_text(
                render_join_gate(update.effective_user.first_name or "there"),
    ...
    if data == "tool_bulk":
        ...
        await query.edit_message_text(
            f"📤  *Bulk Check Mode*\n{LINE_HEAVY}\n"
            f"Send a `.txt` file with one Device ID per line.\n\n"
    ...
    if data == "tool_single":
        ...
        await query.edit_message_text(
            ...
            f"Send the Device ID as a text message.",
            parse_mode="Markdown"
        ); return
    ...
    if data.startswith("gcash_sel:"):
        ...
        await query.edit_message_text(
            ...
            f"Admin will approve within 5–30 minutes.",
            parse_mode="Markdown"
        ); return
    ...
    if data.startswith("adm_"):
        ...
        if data == "adm_running" or data.startswith("adm_run_refresh:"):
            ...
            if target_uid is not None:
                ...
                if not match:
                    await query.answer("Job already finished.", show_alert=True)
                    target_uid = None

            if target_uid is not None:
                _, rjob = match[0]
                s = rjob["stats"].snapshot()
                ...
                txt = (
                    f"⚡  *SESSION — `{target_uid}`*\n"
                    f"{LINE_HEAVY}\n"
                    f"👤 User     : @{uname}\n"
                    f"🕐 Started  : `{start_str}`\n"
                    f"{LINE_HEAVY}\n"
                    f"`{_bar(s['checked'] / max(s['total'],1) * 100)}` "
                    f"*{s['checked']/max(s['total'],1)*100:5.1f}%*\n\n"
                    f"📦 Progress : `{s['checked']}/{s['total']}`\n"
                    f"✅ Valid    : `{s['hits']}`\n"
                    f"🚫 Banned   : `{s['banned']}`\n"
                    f"❌ Invalid  : `{s['failed']}`\n"
                    f"{LINE_HEAVY}\n"
                    f"⚡ Rate     : `{s['live_rate']:.1f}/s`  (avg `{s['avg_rate']:.1f}/s`)\n"
                    f"⏱ Elapsed  : `{_fmt_secs(s['elapsed'])}`\n"
                    f"⏳ ETA      : `{_fmt_secs(s['eta'])}`\n"
                    f"🧵 Threads  : `{rjob.get('threads','?')}`\n"
                    f"{LINE_HEAVY}\n"
                    f"👑 {BRAND}"
                )
                kb = InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔄 Refresh", callback_data=f"adm_run_refresh:{target_uid}")],
