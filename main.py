...
/check  /bulk      → player info (NO ban)
/ban    /banbulk   → ban status only
/status /stop /redeem /mykey /admin /help
"""
...
# ================== CARD RENDERER ==================
def _esc(s):
    return html.escape(str(s if s not in (None, "") else "N/A"))

def render_card(did: str, p: Dict) -> str:
    acc   = p.get("player_id", "?")
    zone  = p.get("server", "?")
    nick  = p.get("nickname", "N/A")
    level = p.get("level", "N/A")
    heroes= p.get("hero_count", "N/A")
    skins = p.get("skin_count", "N/A")

    cur   = p.get("current_rank") or "Unranked"
    high  = p.get("high_rank") or "N/A"
    coll  = p.get("collector_tier") or "N/A"
    cpt   = p.get("collector_point")
    cpt_s = f"{int(cpt):,}" if isinstance(cpt, (int, float)) and cpt > 0 else "N/A"

    loc   = p.get("location") or "NOT FOUND"
    ll    = p.get("last_login") or "N/A"
    squad = p.get("squad") or "N/A"
    wr    = p.get("win_rate", "N/A")
...
def render_job(job: Job) -> str:
    ...
    title = "BAN SCAN" if job.mode == "ban" else "SCANNING"
            ...
            try:
                await bot.send_document(chat_id=chat_id,
                    ...
                    caption=(f"✅ <b>Ban scan done</b>\n"
                             ...
                             f"✅ Not banned: <b>{job.valid}</b> │ "
                             f"❌ Failed: <b>{job.invalid}</b>\n"
                             f"⏱ {job.elapsed:.1f}s"), parse_mode=ParseMode.HTML)
            except Exception as e: log.warning("send_document failed: %s", e)
...
async def cmd_start(msg: Message, command: CommandObject):
    ...
    txt = (f"👋 <b>Welcome, {html.escape(u.first_name or 'user')}!</b>\n\n"
           ...
           "• <code>/check &lt;id&gt;</code> — full player info\n"
...
async def cmd_help(msg: Message):
    await msg.answer(
        f"📖 <b>Help</b>\n\n"
        f"🔍 <code>/check and_xxxx…</code> — full player card\n"
        f"📦 <code>/bulk</code> → send .txt of IDs — player info ({BULK_THREADS} threads)\n"
        f"🚫 <code>/ban and_xxxx…</code> — ban status only\n"
        f"📦 <code>/banbulk</code> → send .txt of IDs — ban scan ({BULK_THREADS} threads)\n"
...
async def cmd_ban(msg: Message, command: CommandObject):
    ...
    if ban.get("banned"):
        ...
        await ph.edit_text(
            f"✅ <b>NOT BANNED</b>\n{LINE_HEAVY}\n"
            f"┗ 🆔 Device: <code>…{html.escape(did[-12:])}</code>\n"
            f"🟢 This account is in good standing.")
...
async def cmd_bulk(msg: Message):
    ...
    await msg.answer(f"📦 <b>Bulk player check</b>\nSend a <b>.txt</b> file "
                     f"(one device ID per line).\n⚡ {BULK_THREADS} threads.\n"
                     f"<i>Note: no ban check — use /banbulk for that.</i>")
...
async def on_document(msg: Message):
    ...
    ph = await msg.answer(f"🚀 Starting <b>{'ban' if mode=='ban' else 'player'}</b> check of "
                          f"<b>{len(ids)}</b> IDs…")
    if mode == "ban":
