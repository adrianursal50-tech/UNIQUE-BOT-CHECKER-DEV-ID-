...
Rich player card + accurate ban check (PID 20001) + BF kicker + live stats.
"""
...
async def run_bulk_ban(bot, chat_id, message_id, user_id, device_ids, input_name="input.txt"):
    ...
    async def one(did):
        ...
        async with sem:
            ...
            job.checked += 1
            if res.get("status") == "success":
                ban = res["ban"]; state = ban.get("state", "clear"); job.registered += 1
                if state == "banned":
                    ...
                    job.unregistered += 1
                    job.results.append(save_ban_line(did, ban))
                    job.latest.append(fmt_line(False, did, str(ban.get("label","UNKNOWN"))[:60]))
                elif state == "unregistered":
                    job.unregistered += 1
                    job.results.append(save_ban_line(did, ban))
                    job.latest.append(fmt_line(False, did, "Unregistered"))
                else:  # clear
                    job.valid += 1
                    job.results.append(save_ban_line(did, ban))
            ...
            try:
                await bot.send_document(chat_id=chat_id,
                    ...
                    caption=(f"✅ <b>Ban scan done</b>\n"
                             ...
                             f"⚠️ Unregistered/Unknown: <b>{job.unregistered}</b> │ "
                             f"❌ Invalid: <b>{job.invalid}</b>\n"
...
async def cmd_check(msg: Message, command: CommandObject):
    ...
    ph = await msg.answer("🔍 <i>Fetching full info (may take up to 30s)…</i>")
...
# ---------- /ban : tri-state (banned / clear / unknown / unregistered) ----------
...
async def cmd_ban(msg: Message, command: CommandObject):
    ...
    ph = await msg.answer("🚫 <i>Checking ban status… (up to 25s per try)</i>")
    res = await asyncio.to_thread(check_ban_only, did)

    # Format / connection failure
    if res.get("status") != "success":
    ...
    state = ban.get("state", "clear" if not ban.get("banned") else "banned")
    ...
    if state == "banned":
        await ph.edit_text(
            ...
            f"┣ ⏱ Duration: <b>{html.escape(str(ban.get('remaining','?')))}</b>\n"
            f"┗ 🔢 Code    : <code>{html.escape(str(ban.get('reason_raw','?')))}</code>")
    elif state == "unknown":
        await ph.edit_text(
            f"⚠️ <b>UNKNOWN</b>\n{LINE_HEAVY}\n"
            f"┣ 🆔 Device  : <code>…{html.escape(did[-12:])}</code>\n"
            f"┣ ℹ️ Status  : <b>Could not determine ban status</b>\n"
            f"┗ 📝 Detail  : <i>{html.escape(str(ban.get('label','no 20001 response')))}</i>\n\n"
            f"<i>Server didn't return PID 20001. This is NOT a clean result — try again.</i>")
    elif state == "unregistered":
        await ph.edit_text(
            f"❌ <b>UNREGISTERED</b>\n{LINE_HEAVY}\n"
            f"┗ 🆔 Device: <code>…{html.escape(did[-12:])}</code>\n"
            f"This device has no MLBB account.")
    else:  # clear
        await ph.edit_text(
            f"✅ <b>NOT BANNED</b>\n{LINE_HEAVY}\n"
            f"┗ 🆔 Device: <code>…{html.escape(did[-12:])}</code>\n"
...
async def cmd_banbulk(msg: Message):
    ...
    await msg.answer(f"🚫 <b>Bulk ban scan</b>\nSend .txt (one ID per line).\n⚡ {BULK_THREADS} threads.\n"
                     f"<i>Uses accurate PID 20001 method — can take a few minutes.</i>")
