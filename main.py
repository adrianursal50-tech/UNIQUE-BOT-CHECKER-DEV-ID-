#!/usr/bin/env python3
# ===================================================================
# PREMIUM DEVID SEKER - TELEGRAM BOT v3.3
# JSON output + login debug logging
# ===================================================================
...
class BaseConnection:
    ...
    def recv_data(self) -> Tuple[Optional[int], Optional[SdpStruct]]:
        try:
            ...
            log.warning(f"[RX] ctype={ctype} size={size} decoded_len={len(data)}")
            log.warning(f"[RX] first32={data[:32].hex()}")
            log.warning(f"[RX] last16 ={data[-16:].hex()}")

            frame = SdpStruct(data)

            log.warning(f"[RX] frame keys={list(frame.keys())}")
            for k in list(frame.keys()):
                v = frame[k]
                log.warning(f"[RX]   tag {k}: {type(v).__name__} = {repr(v)[:120]}")
...
class GameLogin(BaseConnection):
    ...
    def run(self) -> Tuple[Optional[int], Optional[int], str]:
        try:
            ...
            log.warning(f"[LOGIN] pid={pid} res_type={type(res).__name__}")
            if res is not None:
                log.warning(f"[LOGIN] res keys={list(res.keys())}")
                for k in list(res.keys())[:20]:
                    v = res[k]
                    log.warning(f"[LOGIN]   tag {k}: {type(v).__name__} = {repr(v)[:120]}")
...
async def post_init(app: Application) -> None:
    ...
    log.info(f"🚀 {BOT_NAME} v{BOT_VERSION} online — debug logging ON")
