...
# Original format preserved. Lookup API only fills N/A fields.
# ===================================================================
...
# ────────────────────────────────────────────────────────────────
# LOOKUP API  (fills N/A values only)
# ────────────────────────────────────────────────────────────────
...
def apply_lookup_fallback(player_data: dict, full: Dict[str, Any]) -> dict:
    """
    Fill only fields that came back N/A / empty / zero.
    Does not add new fields. Does not change existing values.
    """
    if full.get("status") != "success":
        return player_data

    # Nickname — replace 'N/A', 'unknown', 'guest', 'Player_XXX'
    ...
    # Current rank — if 'Unranked' or blank
...
def process_detail(device_id: str, account_id: int, zone_id: int, out_dir: str = None) -> Tuple[bool, Optional[dict]]:
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            # ── Fill N/A fields from lookup API (no new fields) ──
            try:
    ...
    if awaiting_single and auth:
        ...
        try:
            with GameConnection(device_id=device_id) as conn:
                ...
                # Re-read from player_data (possibly updated)
                nick         = player_data.get('nickname', nick)
                level        = player_data.get('level', level)
                skin_cnt     = player_data.get('skin_count', skin_cnt)
                hero_cnt     = player_data.get('hero_count', hero_cnt)
                cur_rank     = player_data.get('current_rank', 'Unranked')
