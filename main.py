#!/usr/bin/env python3
# ===================================================================
# DEV ID Checker — Telegram Bot v4.0  ◈  SHIN CYBER EDITION
# -------------------------------------------------------------------
#   • Single check + Bulk check
#   • Device ID GENERATOR (and_ / ios_ mixed, unique)
#   • Device ID VALIDATOR (format only, no lookup)
...
def generate_device_id(platform: Optional[str] = None) -> str:
    """
    Generate a unique MLBB-style device ID.
    Format: {prefix}_{32 hex}{24 alnum}-{uuid4}
    platform: "and" | "ios" | "mix" | None
    """
    ...
    # 32 hex chars (imei md5 style)
    hex32 = ''.join(random.choices(_HEX, k=32))
    # 24 alnum chars (android id style)
...
def validate_device_id(did: str) -> Tuple[bool, str]:
    """
    Format-only validator for MLBB device IDs.
    Returns (is_valid, reason).
    NO info lookup — purely structural checks.
    """
    ...
    if not did.startswith(("and_", "ios_")):
        return False, "Missing and_ / ios_ prefix"

    body = did[4:]  # after prefix

    # Length bounds (real IDs are ~85-100 chars after prefix)
    if len(body) < 60:
        return False, f"Body too short ({len(body)} chars, min 60)"
    if len(body) > 120:
        return False, f"Body too long ({len(body)} chars, max 120)"

    # Must start with 32 hex chars (imei md5)
    if not re.match(r'^[0-9a-f]{32}', body):
        return False, "Missing 32-char hex prefix (imei md5)"

    # Must contain at least one dash (UUID separator)
    if "-" not in body:
        return False, "Missing UUID separator (dash)"

    # Character class check — only alnum + dashes after the hex
    rest = body[32:]
    if not re.match(r'^[0-9a-zA-Z\-]+$', rest):
        return False, "Invalid characters in body"

    # UUID part should be at the end (8-4-4-4-12 or 4-4-4-4-12 pattern)
    if not re.search(r'[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$', body):
        return False, "Malformed UUID suffix"

    return True, "Valid MLBB device ID"
...
def render_welcome(name: str, is_admin: bool, expiry: str, checked: int, hits: int) -> str:
    ...
    return (
        ...
        f"┣ 🎲 Generate IDs → create new device IDs\n"
        f"┣ ✅ Validate → check format of IDs\n"
...
def render_generate_menu(platform: str = "mix") -> str:
...
def render_generate_result(count: int, ids: List[str], platform: str, preview: int = 8) -> str:
    icon = {"and":"🤖","ios":"🍎","mix":"🎲"}.get(platform, "🎲")
    lines = [
        f"✅  *GENERATED {count} ID(S)*",
        LINE_HEAVY,
        f"┣ {icon} Platform : `{platform.upper()}`",
        f"┗ 📄 Total    : `{count}`",
        LINE_HEAVY, "",
...
def render_validate_menu() -> str:
    return (
        f"{banner_line()}\n{LINE_HEAVY}\n"
        f"◇  *DEVICE ID VALIDATOR*  ◇\n"
        f"{LINE_HEAVY}\n"
        f"┣ 🔍 Checks : prefix, length, hex, UUID\n"
        f"┣ 📎 Input  : text or `.txt` file\n"
        f"┣ 📄 Bulk   : up to `{MAX_VALIDATE_BULK:,}` per batch\n"
        f"┗ ⚡ Speed  : instant (no network lookup)\n"
        f"{LINE_HEAVY}\n"
        f"_Send one or more device IDs (one per line)._"
        f"\n{LINE_HEAVY}\n👑 {BRAND}"
    )

def render_validate_result(valid_count: int, invalid_count: int,
                            valid_list: List[str], invalid_list: List[Tuple[str, str]],
                            preview: int = 10) -> str:
    ...
    lines = [
        ...
        LINE_HEAVY,
        f"`{_bar(pct, 22)}` *{pct:.1f}%*",
        LINE_HEAVY, "",
    ]
    if valid_list:
        lines.append("*✅ VALID:*")
        for did in valid_list[:preview]:
            lines.append(f"  ↳ `{did}`")
        if len(valid_list) > preview:
            lines.append(f"  _… and {len(valid_list) - preview} more_")
        lines.append("")
    if invalid_list:
        lines.append("*❌ INVALID:*")
        for did, reason in invalid_list[:preview]:
            safe = did.replace("`", "'")[:50]
            lines.append(f"  ↳ `{safe}`")
            lines.append(f"     ⚠️ _{_esc(reason)}_")
        if len(invalid_list) > preview:
            lines.append(f"  _… and {len(invalid_list) - preview} more_")
        lines.append("")
    lines.append(LINE_HEAVY)
    lines.append(f"👑 {BRAND}")
    return "\n".join(lines)
...
class GameConnection(BaseConnection):
    ...
    def login_to_login_server(self):
        ...
        for _ in range(4):
            ...
            if pid == 2 and res:
                ...
                log.info(f"[LOGIN] acc={self.account_id} zone={self.zone_id}")
                return True
            ...
            try:
                ...
                for _ in range(8):
                    ...
                    if pid == 6 and res:
                        ...
                        if host and port:
                            self.game_host = host; self.game_port = int(port)
                            return True
    ...
    def lookup_player(self, search_value):
        ...
        for attempt in range(2):
            ...
            time.sleep(0.6)
        log.warning(f"lookup_player empty — pids seen: {seen}")
        return None
...
def process_detail(device_id, account_id, zone_id, want_debug=False):
    try:
        with GameConnection(device_id=device_id) as conn:
            ...
            if not result:
                seen = [r.get("resp_pid") for r in conn.debug_responses if r.get("label") == "lookup_player"]
                return False, None, f"lookup_empty (pids seen: {seen})", debug_bundle

            try: ban_stat = conn.check_ban_status()
            except Exception as e:
                log.warning(f"ban check failed: {e}"); ban_stat = "NORMAL"
...
def run_bulk_job(job, loop, app):
    ...
    def worker(did):
        ...
        if not acc or not zone:
            stats.inc("checked")
            if 'ban' in str(st).lower(): stats.inc("banned")
            else: stats.inc("failed")
            upd(); return
        ok = pd = reason = None
        for _ in range(2):
            ok, pd, reason, _ = process_detail(did, acc, zone)
            if ok and pd: break
            time.sleep(1.0)
        stats.inc("checked")
        if ok and pd:
            ...
            stats.inc("failed")
            hq.put(line_error(did, reason or "check failed"))
        upd()
...
def main_menu_kb(include_admin: bool = False):
    rows = [
        ...
        [InlineKeyboardButton("✅  Validate IDs", callback_data="tool_validate")],
...
async def cmd_help(update, ctx):
    ...
    await update.message.reply_text(
        ...
        "┣ 🎲 Generate — create random device IDs\n"
        "┣ ✅ Validate — check device ID format\n"
        "┣ 📊 Statistics — rank counters\n"
        "┗ 📖 Menu — /menu\n\n"
...
# ════════════════════════════════════════════════════════════════
# GENERATOR + VALIDATOR INPUT PROCESSORS
# ════════════════════════════════════════════════════════════════

async def _process_generate(update, ctx, count_str, platform):
    """Generate N unique device IDs and return preview + file."""
    try:
    ...
    try:
        await update.effective_message.reply_document(
            ...
            caption=f"📄  *{len(ids)} device IDs* — platform: `{platform}`",
            parse_mode="Markdown")
    except Exception as e:
        log.error(f"generate file send: {e}")


async def _process_validate(update, ctx, text_or_data):
    """Validate device IDs (format only). Send preview + file for large batches."""
    ...
    if not lines:
        await update.effective_message.reply_text("❌ No device IDs provided.")
        return
    ...
    invalid_list = []
    for did in lines:
        ok, reason = validate_device_id(did)
        if ok:
            valid_list.append(did)
        else:
            invalid_list.append((did, reason))

    total = len(valid_list) + len(invalid_list)
    if total == 0:
        await update.effective_message.reply_text("❌ No device IDs found.")
        return

    # Build the result text
    result_text = render_validate_result(
        len(valid_list), len(invalid_list),
        valid_list, invalid_list, preview=10
    )
    if truncated:
        result_text += f"\n\n⚠️ _Truncated to first {MAX_VALIDATE_BULK}_"

    # If result is short enough, send inline; else attach file
    if total <= 30:
        await update.effective_message.reply_text(result_text, parse_mode="Markdown")
        return

    # Send short summary inline + full report as file
    short_txt = (
        f"✅  *VALIDATION COMPLETE*\n{LINE_HEAVY}\n"
        f"┣ 📄 Total   : `{total}`\n"
    ...
    await update.effective_message.reply_text(short_txt, parse_mode="Markdown")

    # Build full report
    report_lines = [
        "SHIN DevID Checker — Validation Report",
        "=" * 50,
        f"Total   : {total}",
        f"Valid   : {len(valid_list)}",
        f"Invalid : {len(invalid_list)}",
    ...
    for did in valid_list:
        report_lines.append(did)
    report_lines += ["", "[INVALID IDs]"]
    for did, reason in invalid_list:
        report_lines.append(f"{did} | {reason}")
    ...
    try:
        await update.effective_message.reply_document(
            document=InputFile(bio, filename=f"validation_report_{ts}.txt"),
            caption=f"📄  *{len(valid_list)} valid* / *{len(invalid_list)} invalid*",
            parse_mode="Markdown")
    except Exception as e:
        log.error(f"validate file send: {e}")
...
async def handle_document(update, ctx):
    ...
    # Validate flow
    if wants_validate:
        await _process_validate(update, ctx, bytes(data))
        with job_lock:
            active_jobs.get(uid, {}).pop("awaiting_validate_input", None)
        return

    # Bulk check flow
    if not wants_bulk:
        await update.message.reply_text(
            "Use /start → 🔥 *Bulk Check* first, or /validate for ID validation.",
            parse_mode="Markdown"); return
...
async def on_text(update, ctx):
    ...
    if awaiting_gen and auth:
        ...
        await _process_generate(update, ctx, text, gen_platform)
        return
    ...
    if awaiting_validate and auth:
        with job_lock:
            active_jobs.get(uid, {}).pop("awaiting_validate_input", None)
        await _process_validate(update, ctx, text)
        return
...
async def handle_callback(update, ctx):
    ...
    if data.startswith("gen_set:"):
        ...
        with job_lock:
            ...
            active_jobs[uid].pop("awaiting_generate_count", None)
            active_jobs[uid]["awaiting_generate_count"] = True
        icon = {"and":"🤖","ios":"🍎","mix":"🎲"}.get(platform, "🎲")
        await q.edit_message_text(
            f"◇  *GENERATOR SETUP*  ◇\n{LINE_HEAVY}\n"
...
async def post_init(app):
    cmds = [
        ...
        BotCommand("validate", "Validate device ID format"),
...
def main():
    ...
    log.info("🔥 Starting SHIN Cyber Edition v4.0...")
    try:
