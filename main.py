...
# Fixed: recv_data handles str-typed bodies (FAIL PID: 2)
# ===================================================================
...
class BaseConnection:
    ...
    # ── FIXED recv_data: tolerates str-typed body at tag 6/5 ──
...
def _build_player_data_from_raw(device_id, account_id, zone_id, conn,
    ...
    if isinstance(pd_raw, str):
        try: pd_raw = pd_raw.encode('utf-8')
        except Exception: pd_raw = None
    if isinstance(pd_raw, bytes):
        try: pd_raw = SdpStruct(pd_raw)
        except Exception: pd_raw = None
