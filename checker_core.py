...
Ban check is SEPARATE from player check.
"""
...
class BaseConnection:
    ...
    def recv_data(self):
        ...
        try:
            ...
            # Fallback: the packet itself IS the payload
            # (some servers send PID 2 with fields 0/1/2 at the top level)
            if any(k in res for k in (0, 1, 2, 10)):
                return pid, res
...
class GameConnection(BaseConnection):
    ...
    # ---------- LOGIN (loop, skip interim PIDs, validate payload) ----------
    def login_to_login_server(self) -> bool:
        ...
        for _ in range(10):
            ...
            if pid == -1:
                saw_timeout = True
                continue
            if pid == 20001:                 # interim ban info — skip
                continue
            if pid != 2:                     # anything else — skip
                continue
            if not res:                      # PID 2 with empty body — retry
                continue

            acc = res.get(0)
            if acc is None:                  # no account id — retry
                continue
        ...
        if saw_timeout:
            self.last_login_err = "Login timeout — server didn't respond"
        elif saw_close:
            self.last_login_err = "Server closed connection"
        else:
            self.last_login_err = f"Login failed (last PID: {last_pid})"
        return False
...
def _setup_connection(device_id: str) -> GameConnection:
    """Login + get_server + handshake. Raises ConnectionError with a friendly reason."""
...
def check_ban_only(device_id: str, retries: int = 3) -> Dict[str, Any]:
    """Ban status only. NO player lookup. Fresh connection each attempt."""
    device_id = (device_id or "").strip()
    ok, reason = validate_device_id(device_id)
    if not ok:
