...
With retry + backoff to prevent false negatives on valid accounts.
"""
...
class GameConnection(BaseConnection):
    ...
    def _drain_queue(self):
        try: self.socket.settimeout(0.3)
...
def _setup_connection(device_id: str) -> GameConnection:
    """Login + get_server + handshake. Uses fresh socket."""
...
# ============================================================
# CLASSIFIERS — helps decide whether to retry or bail out
# ============================================================
_PERMANENT_FAILURES = (
    "guest / unregistered",
    "invalid format",
...
def _is_transient_failure(err: str) -> bool:
    ...
    return any(t in e for t in (
        "login failed", "server resolve failed", "handshake failed",
        "timeout", "connection", "socket", "no account data returned",
    ))
...
def check_device_id(device_id: str, retries: int = None) -> Dict[str, Any]:
    """Full player check with retry + exponential backoff."""
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            # If lookup came back empty, it might be transient — retry ONCE
            # by re-doing the full flow before declaring unregistered.
            ...
            if not result:
                # All retries exhausted → treat as unregistered (no more retries)
                try: conn.cleanup()
                except Exception: pass
                return {"status": "error", "device_id": device_id,
            ...
            # Guest / invalid → don't waste time retrying
...
def check_ban_only(device_id: str, retries: int = None) -> Dict[str, Any]:
    """Ban check with retry + exponential backoff."""
    if retries is None: retries = CHECK_RETRIES
    device_id = (device_id or "").strip()
    ok, reason = validate_device_id(device_id)
    if not ok:
        return {"status": "error", "device_id": device_id,
                "error": f"Invalid format: {reason}", "permanent": True,
                "ban": {"banned": False, "label": "INVALID"}}
    last_err = "unknown"
    for attempt in range(retries):
        conn = None
        try:
            ...
            if _is_permanent_failure(last_err):
                return {"status": "error", "device_id": device_id,
                        "error": last_err, "permanent": True,
                        "ban": {"banned": False, "reason": "", "remaining": "",
                                "label": "UNREGISTERED"}}
            time.sleep(RETRY_BACKOFF[min(attempt, len(RETRY_BACKOFF)-1)])
            continue
        except Exception as e:
