...
Ban check now uses PID 20001 from auto_cek_ban.py (more accurate).
"""
...
SOCK_CONNECT    = float(os.environ.get("MLBB_CONNECT_TIMEOUT", "12.0"))
...
BAN_WAIT        = float(os.environ.get("MLBB_BAN_WAIT",       "25.0"))  # ← wait for 20001
...
# ═══════════════════════════════════════════════════════════════
# BAN REASON MAP — codes 21-49 from auto_cek_ban.py (primary)
#                    codes 1-20 from prior reference (fallback)
# ═══════════════════════════════════════════════════════════════
BAN_REASON_MAP: Dict[str, str] = {
    # ── auto_cek_ban.py (authoritative for 2024+ MLBB) ──
    "21": "Cheats",
...
BAN_TYPE_MAP = {0:"Temporary",1:"Permanent",2:"Permanent",3:"Suspension",4:"Restriction",5:"Warning"}
def translate_ban_type(raw) -> str:
    try: return BAN_TYPE_MAP.get(int(raw), f"Type {raw}")
    except Exception: return "Unknown"

# ═══════════════════════════════════════════════════════════════
# PID 20001 PARSER — walks nested dict/list, extracts ban fields
# ═══════════════════════════════════════════════════════════════
def parse_ban_20001(res) -> Optional[Dict[str, Any]]:
    """
    Parse PID 20001 for ban data.
    Returns dict with ban info if banned, None if the packet is a CLEAN signal.
    """
    if not res:
        return None
    reason_code = None
    ...
    # No ban fields at all → this 20001 is a CLEAN signal
    if reason_code is None and day is None:
        return None

    info = {
        "ban_reason": reason_code or "?",
        "reason_name": translate_ban_reason(reason_code) if reason_code else "Banned",
    }
    if day is not None:
        info["duration_str"] = f"{day}d {hour or '00'}:{minute or '00'}:{sec or '00'}"
    else:
        info["duration_str"] = "Permanent"
    return info
...
def fmt_ban_remaining(end_ts) -> str:
    try: end_ts = int(end_ts or 0)
    except Exception: end_ts = 0
    if end_ts <= 0: return "Never (permanent)"
    now = int(time.time())
    if end_ts <= now: return "Expired"
    secs = end_ts - now
...
def _first_positive_int(*vals) -> int:
    for v in vals:
        if isinstance(v, bool): continue
        if isinstance(v, int) and v > 0: return v
    return 0

def _first_ts(*vals) -> int:
    for v in vals:
        if isinstance(v, bool): continue
...
class GameConnection(BaseConnection):
    ...
    def login_to_login_server(self) -> bool:
        ...
        for _ in range(LOGIN_RETRIES):
            ...
            if pid == 2 and res:
                ...
                self.login_ban_end_ts = _first_ts(res.get(20, 0), res.get(24, 0), res.get(26, 0))
                self.login_ban_reason = _first_positive_int(res.get(21, 0), res.get(23, 0), res.get(25, 0))
                self.login_ban_type = _first_positive_int(res.get(22, 0), res.get(31, 0))
    ...
    # ── OLD handshake — used by /check (player info) ──
    ...
    # ══════════════════════════════════════════════════════════
    # NEW BAN CHECK — from auto_cek_ban.py
    # Sends 10001 + 10101 together on game server, waits for 20001.
    # ══════════════════════════════════════════════════════════
    def do_ban_check(self) -> Dict[str, Any]:
        """
        Returns:
            {"state": "banned", "banned": True, "reason": ..., "remaining": ..., "label": ...}
            {"state": "clear",  "banned": False, "label": "Not Banned"}
            {"state": "unknown","banned": False, "label": "..."}   ← never assume clean
        """
        # (Re)connect to the game server
        self.cleanup()
        self.host, self.port = self.game_host, self.game_port
        try: self.connect()
        except Exception as e:
            return {"state": "unknown", "banned": False, "label": f"Game connect failed: {e}"}

        # Send handshake + ban query TOGETHER (this is the key difference)
        try:
            ...
            return {"state": "unknown", "banned": False, "label": f"Send failed: {e}"}
        ...
        while time.time() < deadline:
            if self._should_abort():
                return {"state": "unknown", "banned": False, "label": "aborted"}

            # Per-read timeout scales down as the deadline approaches
            remaining = max(0.5, deadline - time.time())
            try: self.socket.settimeout(min(10.0, remaining))
            except Exception: pass

            pid, res = self.recv_data()

            if pid is None:
                return {"state": "unknown", "banned": False,
                        "label": "Server closed before 20001"}

            if pid == -1:
                # read timeout — keep looping until overall deadline
                continue

            # ══ THE definitive packet ══
            if pid == 20001:
                info = parse_ban_20001(res)
                if info:
                    return {
                        "state": "banned",
                        "banned": True,
                        "reason": info["reason_name"],
                        "reason_raw": info["ban_reason"],
                        "remaining": info["duration_str"],
                        "label": (f"Banned | Reason: {info['reason_name']} "
                                  f"(code {info['ban_reason']}) | Duration: {info['duration_str']}"),
                    }
                # 20001 with no ban fields → clean
                return {"state": "clear", "banned": False, "label": "Not Banned"}

            # Trigger role info request (same as auto_cek_ban)
            if pid == 10002 and not role_sent:
                try:
                    self.send_data(10003, SdpStruct({
                        0: self.account_id, 1: self.session_key,
                        2: self.zone_id, 3: CLIENT_VERSION,
                        4: self.channel, 5: self.device_id}))
                except Exception: pass
                role_sent = True
                continue

            # 10004 = lobby marker; keep waiting for 20001 (do NOT mark clean)
            if pid == 10004:
                continue

            # Any other packet — ignore, keep waiting
            continue

        # Deadline reached without seeing 20001 → UNKNOWN
        return {"state": "unknown", "banned": False,
                "label": f"No 20001 within {int(BAN_WAIT)}s"}
    ...
    def get_role_info(self, role_id, zone_id) -> Optional[SdpStruct]:
        try:
    ...
    def get_skin_role_info(self, role_id, zone_id) -> Optional[SdpStruct]:
        for _ in range(3):
            try:
                ...
                while to < 3:
                    ...
                    if pid == -1: to += 1; continue
                    if pid == 10144: return res
                    if pid == 20001: continue
            except Exception: pass
        return None

    def get_v2l_status(self, role_id, zone_id) -> Optional[Dict[str, Any]]:
        for _ in range(2):
            try:
                ...
                while to < 3:
                    ...
                    if pid is None: break
                    if pid == -1: to += 1; continue
                    if pid == 10208 and res: return {"src": 10208, "data": dict(res)}
                    if pid == 20001: continue
                ...
                while to < 3:
                    ...
                    if pid is None: break
                    if pid == -1: to += 1; continue
                    if pid in (10146, 10160) and res: return {"src": pid, "data": dict(res)}
                    if pid == 20001: continue
            except Exception: pass
        return None
...
def _setup_connection(device_id: str) -> GameConnection:
    """Login + get_server + handshake. Used by /check."""
...
# ═══════════════════════════════════════════════════════════════
# BAN CHECK — replaced with the PID-20001 approach
# ═══════════════════════════════════════════════════════════════
def check_ban_only(device_id: str, retries: int = None) -> Dict[str, Any]:
    """
    Ban check via PID 20001 on the game server (accurate version).
    Returns:
        {"status": "success", "ban": {"banned": bool, "state": "banned"|"clear"|"unknown", ...}}
    """
    if retries is None: retries = CHECK_RETRIES
    device_id = (device_id or "").strip()
    ok, reason = validate_device_id(device_id)
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            # 3) Connect to game server + send 10001 + 10101 + wait for 20001
            ban = conn.do_ban_check()
            conn.cleanup()
            last_ban = ban

            # If it's unknown and we still have retries → back off and retry
...
def save_ban_line(did: str, ban: Dict[str, Any]) -> str:
    ...
    if state == "unknown":
        return f"{did} | UNKNOWN | {ban.get('label','Could not determine ban status')}"
