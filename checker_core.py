...
SOCKET_TIMEOUT   = float(os.environ.get("MLBB_SOCK_TIMEOUT", "1.5"))
CONNECT_TIMEOUT  = float(os.environ.get("MLBB_CONNECT_TIMEOUT", "1.5"))
...
class GameConnection(BaseConnection):
    ...
    def login_to_login_server(self) -> bool:
        ...
        if pid == 2 and res:
            ...
            self.zone_id = res[2][0]; err = res.get(10, 0)
...
def check_device_id(device_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not conn.login_to_login_server():
            conn.cleanup(); return {"status":"error","device_id":device_id,"error":"Login failed"}
        aid = conn.account_id; bfl = conn.ban_status; bel = conn.ban_end_ts
        if not aid:
            conn.cleanup(); return {"status":"error","device_id":device_id,"error":"No account linked"}
        if not conn.get_game_server():
            conn.cleanup(); return {"status":"error","device_id":device_id,"error":"Server resolve failed"}
        if not conn.connect_to_game_server():
            conn.cleanup(); return {"status":"error","device_id":device_id,"error":"Handshake failed"}
        result = conn.lookup_player(aid, search_type="id"); conn.cleanup()
        if not result:
...
def save_line(did: str, p: Dict[str, Any]) -> str:
    ...
    ban = p.get("ban_status", "N/A"); be = p.get("ban_end", "N/A")
