...
exceptclass GameConnection ImportError:
...
SOCK_READ      = float(os.environ.get("MLBB_SOCK_TIMEOUT", "8.0"))
RECV_CHUNK     = 8192
...
def looks_generated(did: str) -> bool:
    """Heuristic: detect IDs produced by the /generate command (fake UUID)."""
    body = did[4:] if did[:4].lower() in ("and_","ios_") else did
    if len(body) < 60: return False
    tail = body[56:]  # the UUID portion after 32-hex + 24-alnum
    # Real UUIDs are 8-4-4-4-12. Generated ones here look like "1668-46b5-b64d-<12hex>"
    parts = tail.split("-")
    if len(parts) == 4 and len(parts[0]) == 4 and len(parts[1]) == 4 and len(parts[2]) == 4:
        return True
    return False
...
class GameConnection(BaseConnection):
    def __init__(self, device_id: str):
        ...
        self.is_guest = False
        self.last_login_err = ""
        self.debug_pids: List[int] = []   # for troubleshooting

    # ---------- helpers ----------
    @staticmethod
    def _coerce_zone(z) -> int:
        """Server may send zone as int / list / struct / str. Normalise to int."""
    ...
    def _parse_hostport(raw) -> Tuple[Optional[str], Optional[int]]:
        """Robustly extract (host, port) from bytes/str/dict/list/struct."""
        ...
        if isinstance(raw, dict):
            # recursive: any value that looks like "host:port"
            for v in raw.values():
                if isinstance(v, str) and ":" in v:
                    h, p = GameConnection._parse_hostport(v)
                    if h and p: return h, p
            # otherwise try explicit keys
    ...
    def login_to_login_server(self) -> bool:
        ...
        saw_timeout = False; saw_close = False; last_pid = None
        for _ in range(10):
            pid, res = self.recv_data()
            last_pid = pid
            self.debug_pids.append(pid)
            if pid is None:
                saw_close = True; break
            if pid == -1:
                saw_timeout = True
                continue
            if pid == 20001:
            ...
            if pid == 2 and res:
                ...
                self.zone_id = self._coerce_zone(res.get(2))   # ← robust zone
                err = res.get(10, 0)
                if err in (3,4,5,6,100,101,102):
                    self.ban_status = "BANNED"; self.ban_end_ts = res.get(20, 0)
                return True
        if saw_timeout:
            self.last_login_err = "No account found on this device ID"
        elif saw_close:
            self.last_login_err = "Server rejected this device ID"
        else:
            self.last_login_err = "Login failed"
        return False

    # ---------- GET GAME SERVER (robust) ----------
                    ...
                    if isinstance(res, dict):
                        for v in res.values():
                            ...
                            if h and p:
                                ...
                                return True
                    # 3) dump for debugging
                    try:
                        logging.debug("PID 6 payload: %r", dict(res))
                    except Exception: pass
            time.sleep(0.4)
        return False
...
def check_device_id(device_id: str) -> Dict[str, Any]:
    ...
    likely_fake = looks_generated(device_id)
    ...
    for attempt in range(3):
        ...
        try:
            ...
            if not conn.login_to_login_server():
                ...
                last_err = conn.last_login_err or "Login failed"
                conn.cleanup()
                time.sleep(0.8)
                continue

            if not conn.get_game_server():
                pids = conn.debug_pids[-15:]
                last_err = f"Server resolve failed (recent PIDs: {pids})"
                conn.cleanup(); time.sleep(0.8); continue
    ...
    if likely_fake:
        hint = "This ID was probably created by /generate — it has no MLBB account."
        return {"status": "error", "device_id": device_id, "generated": True,
