...
Rich player data + working BF kicker + ban check.
"""
...
LOGIN_TIMEOUT   = float(os.environ.get("MLBB_LOGIN_TIMEOUT", "15.0"))
LOGIN_RETRIES   = int(os.environ.get("MLBB_LOGIN_RETRIES", "4"))
RECV_CHUNK      = 8192
...
BAN_REASON_MAP: Dict[int, str] = {
    0:"None",1:"Cheating / Hack tools",2:"Modified game client",3:"Third-party software",
    4:"Auto-clicker / Bot usage",5:"Bug / Exploit abuse",6:"Illegal top-up",7:"Payment fraud",
    8:"Refund abuse",9:"Unauthorized seller purchase",10:"Account sharing",11:"Account theft",
    12:"Suspicious login activity",13:"Rank boosting / manipulation",14:"Match result manipulation",
    20:"Verbal abuse",21:"Toxic behavior / Harassment",22:"Offensive language",23:"Hate speech",
    24:"Threats / Doxxing",30:"AFK / Passive play",31:"Intentional feeding",32:"Griefing",
    40:"Smurfing / Alt accounts",41:"Multiple account abuse",50:"Impersonation",51:"Report abuse",
    60:"Ban evasion",99:"Multiple violations",100:"Permanent ban — Severe violations",
}
def translate_ban_reason(raw) -> str:
    if raw is None or raw == "": return "Unspecified"
    if isinstance(raw, str) and not raw.strip().isdigit(): return raw.strip()
    try: r = int(raw)
    except (ValueError, TypeError): return str(raw)
    return BAN_REASON_MAP.get(r, f"Code {r}")
...
class GameConnection(BaseConnection):
    ...
    def get_role_info(self, role_id, zone_id) -> Optional[SdpStruct]:
        """Packet 10128 → 10129. Returns role info (squad_id, followers, etc.)"""
        try:
    ...
    def get_skin_role_info(self, role_id, zone_id) -> Optional[SdpStruct]:
        """Packet 10143 → 10144. Returns tag 118 (skin breakdown)."""
        for _ in range(2):
            try:
    ...
    def get_v2l_status(self, role_id, zone_id) -> Optional[Dict[str, Any]]:
        """Packet 10208 → binding info. Fallback: 10145 → 10146/10160."""
        for _ in range(2):
            try:
    ...
    def check_ban_status(self) -> Dict[str, Any]:
        ...
        self._drain_queue()
        try: self.send_data(10101, SdpStruct({0: 0, 2: 2}))
        except ConnectionError:
            return {"banned": False, "reason": "", "remaining": "", "label": "CHECK FAILED"}
        for _ in range(6):
            if self._should_abort(): break
            pid, res = self.recv_data()
            ...
            if pid == 20001 and res and isinstance(res, dict) and 0 in res and isinstance(res[0], dict):
                ...
                if (raw_reason not in (None, "", 0, "0")) or total > 0:
                    remaining = f"{d}d {h}h {m}m {s}s"
                    return {"banned": True, "reason": friendly, "reason_raw": raw_reason,
                            "remaining": remaining,
                            "label": f"Banned | Reason: {friendly} | Remaining: {remaining}"}
                return {"banned": False, "reason": "", "remaining": "", "label": "Not Banned"}
            if pid in (-1, None): break
        # Fallback to login-level ban
        if self.login_ban_flag:
            remaining = fmt_ban_remaining(self.login_ban_end_ts)
            friendly = LOGIN_ERROR_MEANING.get(self.login_error_code, "Banned")
            return {"banned": True, "reason": friendly, "reason_raw": self.login_ban_reason,
                    "remaining": remaining,
                    "label": f"Banned | {friendly} | Remaining: {remaining}"}
        return {"banned": False, "reason": "", "remaining": "", "label": "Not Banned"}
...
def _setup_connection(device_id: str) -> GameConnection:
    ...
    try: conn.connect()
    except Exception as e: raise ConnectionError(f"Cannot reach login server: {e}")
    if not conn.login_to_login_server():
        if conn.is_guest: raise ConnectionError("Guest / unregistered device")
        raise ConnectionError("Login failed")
    if not conn.get_game_server(): raise ConnectionError("Server resolve failed")
    if not conn.connect_to_game_server(): raise ConnectionError("Handshake failed")
    return conn

# ---------------- Extract rich player data ----------------
...
def extract_player_data(result, role_info=None, creation_ts=0, v2l_data=None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Likes / followers / popularity / bio
        likes = 0
    ...
    for _ in range(retries):
        ...
        try:
            ...
            if not result:
                ...
                conn.cleanup(); time.sleep(0.4); continue
            # Fetch extra packets for richer data
...
def check_ban_only(device_id: str, retries: int = 3) -> Dict[str, Any]:
    ...
    if not ok:
        return {"status": "error", "device_id": device_id,
                "error": f"Invalid format: {reason}", "ban": {"banned": False, "label": "INVALID"}}
    last_err = "unknown"
    for _ in range(retries):
        conn = None
        try:
...
def send_session_kick(profile, timeout=4.5) -> Tuple[bool, float, str]:
