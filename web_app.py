"""
MLBB Device ID Checker — Web App v2
Auth + credits + captcha + job persistence + admin panel.
"""
from __future__ import annotations
import os, time, threading, uuid, sqlite3, secrets, json, re, random
...
def user_row_to_dict(row):
    ...
    return {"id": row["id"], "username": row["username"], "credits": row["credits"],
...
def login_required(f):
    @wraps(f)
    def w(*a, **k):
        if not current_user(): return jsonify({"ok": False, "error": "not_logged_in"}), 401
        return f(*a, **k)
    return w
...
def rate_check(ip: str, bucket: str, max_hits: int, window: float) -> bool:
    ...
    with RATE_LOCK:
        ...
        if len(hits) >= max_hits:
            RATE_BUCKETS[key] = hits; return False
...
# ─────────────────────────────────────────────────────────────
# Jobs (memory + SQLite persistence)
# ─────────────────────────────────────────────────────────────
...
def _save_job(job):
    ...
    conn.execute("""
        ...
        ON CONFLICT(id) DO UPDATE SET
            checked=excluded.checked, registered=excluded.registered,
            unregistered=excluded.unregistered, invalid=excluded.invalid,
            valid=excluded.valid, banned=excluded.banned, clean=excluded.clean,
            stopped=excluded.stopped, done=excluded.done, finished=excluded.finished,
            latest=excluded.latest, results=excluded.results
...
def new_job(user_id, kind, total) -> Dict[str, Any]:
    ...
    job = {"id": jid, "user_id": user_id, "kind": kind, "total": total,
           "checked": 0, "registered": 0, "unregistered": 0, "invalid": 0,
           "valid": 0, "banned": 0, "clean": 0, "stopped": False, "done": False,
           "started": int(time.time()), "finished": 0, "latest": [], "results": []}
    with JOBS_LOCK: JOBS_CACHE[jid] = job
    _save_job(job)
    return job

def job_snapshot(jid):
    with JOBS_LOCK: job = JOBS_CACHE.get(jid)
    if not job:
        conn = get_db()
...
def _run_bulk_check(jid, device_ids, threads):
        ...
        with JOBS_LOCK:
            ...
            if st == "registered":
                ...
                if upd["info"].get("state") == "valid":
                    j["valid"] += 1
                    if upd["line"]: j["results"] = (j["results"] + [upd["line"]])[-500:]
    ...
    try:
        with cf.ThreadPoolExecutor(max_workers=threads) as ex: list(ex.map(one, device_ids))
    finally:
        with JOBS_LOCK:
            j = JOBS_CACHE.get(jid)
            if j: j["done"] = True; j["finished"] = int(time.time()); _save_job(j)
...
def _run_bulk_ban(jid, device_ids, threads):
    ...
    def one(did):
        ...
        if res.get("status") == "success":
            ...
            upd["state"] = state; upd["line"] = save_ban_line(did, ban)
            if state == "banned":
...
def spend_credits(user_id, amount, reason) -> bool:
    with CREDIT_LOCK:
        ...
        conn.execute("INSERT INTO credit_log (user_id, amount, reason, created_at) VALUES (?,?,?,?)",
...
def api_signup():
    ...
    try:
        cur = conn.cursor()
        cur.execute("INSERT INTO users (username, password_hash, credits, created_at) VALUES (?,?,?,?)",
                    (username, generate_password_hash(password), FREE_CREDITS, int(time.time())))
        uid = cur.lastrowid
        cur.execute("INSERT INTO credit_log (user_id, amount, reason, created_at) VALUES (?,?,?,?)",
    ...
    conn.close()
    session["user_id"] = uid
    return jsonify({"ok": True, "user": {"id": uid, "username": username, "credits": FREE_CREDITS,
                                          "is_admin": False}})
...
def api_generate():
    d = request.get_json(force=True) or {}
    try: count = int(d.get("count") or 10)
    except Exception: return jsonify({"ok": False, "error": "Invalid count"}), 400
...
def api_bulk_start():
    ...
    if not ids: return jsonify({"ok": False, "error": "No device IDs found"}), 400
...
def api_jobs_mine():
    ...
    rows = conn.execute("""
        ...
        FROM jobs WHERE user_id = ? ORDER BY started DESC LIMIT 20
    """, (u["id"],)).fetchall()
    conn.close()
...
def api_kick_start():
    ...
    did = (d.get("device_id") or "").strip()
    if not did: return jsonify({"ok": False, "error": "Missing device_id"}), 400
    if not spend_credits(u["id"], 5, f"kick:{did[-8:]}"):
        return jsonify({"ok": False, "error": "Not enough credits (need 5)"}), 402
    ...
    def runner():
        try:
            ...
            stats["status"] = "error"; stats["error"] = str(e)
...
def api_admin_stats():
    ...
    jobs_running = conn.execute("SELECT COUNT(*) c FROM jobs WHERE done = 0").fetchone()["c"]
    credits_used = conn.execute(
        "SELECT COALESCE(SUM(ABS(amount)),0) s FROM credit_log WHERE amount < 0"
    ).fetchone()["s"]
    credits_added = conn.execute(
        "SELECT COALESCE(SUM(amount),0) s FROM credit_log WHERE amount > 0"
    ).fetchone()["s"]
    conn.close()
...
def api_admin_users():
    ...
    if q:
        rows = conn.execute("""
            ...
            FROM users WHERE username LIKE ? ORDER BY id DESC LIMIT 200
        """, (f"%{q}%",)).fetchall()
    else:
        rows = conn.execute("""
            SELECT id, username, credits, is_admin, is_banned, created_at, last_login
...
def api_admin_user_update(uid):
    ...
    if action == "add_credits":
        ...
        conn.execute("UPDATE users SET is_banned = 1 WHERE id = ?", (uid,)); conn.commit()
    elif action == "unban":
        conn.execute("UPDATE users SET is_banned = 0 WHERE id = ?", (uid,)); conn.commit()
    elif action == "make_admin":
        conn.execute("UPDATE users SET is_admin = 1 WHERE id = ?", (uid,)); conn.commit()
    elif action == "remove_admin":
        conn.execute("UPDATE users SET is_admin = 0 WHERE id = ?", (uid,)); conn.commit()
    elif action == "reset_password":
        newpw = str(d.get("password") or "")
        if len(newpw) < 6:
            conn.close(); return jsonify({"ok": False, "error": "Password too short"}), 400
        conn.execute("UPDATE users SET password_hash = ? WHERE id = ?",
                     (generate_password_hash(newpw), uid))
        conn.commit()
    else:
        conn.close(); return jsonify({"ok": False, "error": "unknown action"}), 400
    conn.close()
    return jsonify({"ok": True})
